...
class NPC:
    def __init__(self, name, x, y, role, shop_items=None):
        ...
        self.x = x
        self.y = y
        self.role = role  # 'librarian', 'blacksmith', 'enchanter', etc.
        self.size = 16
    ...
    def update(self, dt):
        """NPCs wander around their spawn area"""
        now = time.time()
...
class Room:
    ...
    def create_town_layout(self):
        ...
        self.buildings.append({
            ...
            'has_sign': False,
            'interior': [
                {'type': 'rect', 'x': 20, 'y': 20, 'w': 80, 'h': 60, 'color': '#654321'},
                {'type': 'rect', 'x': 10, 'y': 10, 'w': 20, 'h': 20, 'color': '#8B4513'},
            ]
        })
        
        # Library (top-center) - NO SIGN
        self.buildings.append({
            'type': 'library',
            'x': 600, 'y': 200,
        ...
        # Blacksmith Forge (top-right) - SQUARE with CHIMNEY
        self.buildings.append({
            'type': 'blacksmith',
            'x': 1000, 'y': 200,
            ...
            'has_chimney': True,
            'interior': [
                {'type': 'oval', 'x': 50, 'y': 40, 'w': 30, 'h': 30, 'color': '#FF4500'},
                {'type': 'rect', 'x': 80, 'y': 50, 'w': 40, 'h': 20, 'color': '#696969'},
            ]
        })
        
        # Enchanter Tower (center-left) - NO SIGN
        self.buildings.append({
            'type': 'tower',
            'x': 200, 'y': 500,
            'width': 80, 'height': 180,
        ...
        self.buildings.append({
            ...
            'shape': 'bread',
            'interior': [
                {'type': 'rect', 'x': 50, 'y': 30, 'w': 40, 'h': 40, 'color': '#8B4513'},
                {'type': 'rect', 'x': 10, 'y': 10, 'w': 30, 'h': 20, 'color': '#FFD700'},
            ]
        })
        ...
        # === NPCs ===
        self.npcs.append(NPC("Eldrin", 650, 240, 'librarian', self.get_librarian_items()))
        self.npcs.append(NPC("Gorak", 1050, 240, 'blacksmith', self.get_blacksmith_items()))
        self.npcs.append(NPC("Mystara", 240, 550, 'enchanter', self.get_enchanter_items()))
        self.npcs.append(NPC("Zephyr", 540, 510, 'alchemist', self.get_alchemist_items()))
        self.npcs.append(NPC("Berta", 900, 510, 'chef', self.get_chef_items()))
        self.npcs.append(NPC("Gemma", 260, 810, 'jeweler', self.get_jeweler_items()))
        self.npcs.append(NPC("Marcus", 650, 810, 'trader', SHOP_ITEMS[:10]))
        
        # Named cartographer — sells the Dungeon Map
...
class GameFrame(tk.Frame):
    def __init__(self,parent,player,on_quit_to_menu,dungeon_id=1):
        ...
        self.canvas = tk.Canvas(self, width=WINDOW_W+MAP_PANEL_W, height=WINDOW_H, bg="black")
        ...
        self.canvas.bind('<Button-3>', self.on_right_click)   # right-click to use item
    ...
    def _build_inv_canvas(self, container_win):
        ...
        def redraw():
            ...
            # Section labels
            cv.create_text(EQ_W//2, TOP + GAP//2, text="EQUIPMENT",
    ...
    def on_key_down(self, e):
        ...
        if e.keysym.lower() == 'i':
            self.open_grid_inventory()    # I = standalone grid inventory
    ...
    def draw_minimap(self):
        """Draw the mini-map panel to the right of the game canvas."""
        px = WINDOW_W   # left edge of the panel
        py = 0
        pw = MAP_PANEL_W
        ph = WINDOW_H

        # Dark panel background
        self.canvas.create_rectangle(px, py, px+pw, ph,
                                     fill='#0a0a0a', outline='#333355', width=1)
        self.canvas.create_text(px + pw//2, py + 8,
                                text='MAP', fill='#5555aa',
                                font=('Arial', 8, 'bold'))

        if not self._player_has_map():
            self.canvas.create_text(px + pw//2, ph//2,
                                    text='No map\nequipped',
                                    fill='#444466', font=('Arial', 8), justify='center')
            return
        ...
        if self.dungeon_id == 0:
            self._draw_minimap_town(mx, my, ms)
        else:
            self._draw_minimap_dungeon(mx, my, ms)

    def _draw_minimap_town(self, mx, my, ms):
        ...
        # Grass background
        self.canvas.create_rectangle(mx, my, mx+ms, my+ms,
        ...
        self.canvas.create_oval(ppx-3, ppy-3, ppx+3, ppy+3,
                                fill='#4488ff', outline='white', width=1)

    def _draw_minimap_dungeon(self, mx, my, ms):
        """Mini-map for dungeon — shows full room grid."""
        rows = ROOM_ROWS
        cols = ROOM_COLS
        cw = ms // cols
        ch = (ms - 20) // rows   # leave room label area at bottom
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if is_current:
                    room_fill = '#1a1a2e'
                self.canvas.create_rectangle(rx+1, ry+1, rx+cw-1, ry+ch-1,
                                             fill=room_fill, outline='')

                # Walls (white borders)
                # Top wall — always solid unless row > 0 (has opening)
                if row == 0:
                    self.canvas.create_line(rx, ry, rx+cw, ry, fill='white', width=1)
                else:
                    # Door gap in centre
                    gap = cw // 3
                    self.canvas.create_line(rx, ry, rx+gap, ry, fill='white', width=1)
                    self.canvas.create_line(rx+cw-gap, ry, rx+cw, ry, fill='white', width=1)

                # Bottom wall
                if row == rows - 1:
                    self.canvas.create_line(rx, ry+ch, rx+cw, ry+ch, fill='white', width=1)
                else:
                    gap = cw // 3
                    self.canvas.create_line(rx, ry+ch, rx+gap, ry+ch, fill='white', width=1)
                    self.canvas.create_line(rx+cw-gap, ry+ch, rx+cw, ry+ch, fill='white', width=1)

                # Left wall
                if col == 0:
                    self.canvas.create_line(rx, ry, rx, ry+ch, fill='white', width=1)
                else:
                    gap = ch // 3
                    self.canvas.create_line(rx, ry, rx, ry+gap, fill='white', width=1)
                    self.canvas.create_line(rx, ry+ch-gap, rx, ry+ch, fill='white', width=1)

                # Right wall
                if col == cols - 1:
                    self.canvas.create_line(rx+cw, ry, rx+cw, ry+ch, fill='white', width=1)
                else:
                    gap = ch // 3
                    self.canvas.create_line(rx+cw, ry, rx+cw, ry+gap, fill='white', width=1)
                    self.canvas.create_line(rx+cw, ry+ch-gap, rx+cw, ry+ch, fill='white', width=1)
                ...
                if is_current:
                    ...
                    self.canvas.create_oval(ppx-2, ppy-2, ppx+2, ppy+2,
                                            fill='#4488ff', outline='white', width=1)

        # Room coords label bottom
        self.canvas.create_text(
            mx + ms//2, my + rows*ch + 8,
            text=f'Room ({self.room_row},{self.room_col})',
            fill='#5555aa', font=('Arial', 7)
        )
    ...
    def update_entities(self,dt):
        ...
        for s in self.summons:
            for e in self.room.enemies:
                resolve_overlap(s, e)
        if self.dungeon_id == 0:
            # Check for nearby NPCs (using WORLD coordinates)
            for npc in self.room.npcs:
                npc.update(dt)
            self.nearby_npc = None
            for npc in self.room.npcs:
            ...
            if p.y - player_size < wall_thickness:
                ...
                if self.room_row == 0 and self.room_col == 0 and self.dungeon_id > 0:
                    ...
                    if opening_x_start < p.x < opening_x_end:
                        ...
                        # Get which dungeon they were in to return to correct portal
                        dungeon_portals = {
                            1: (100, 400),    # Forest Temple
                            2: (1300, 250),   # Volcano Depths
                            3: (1300, 650),   # Ice Cavern
                            4: (650, 1000)    # Shadow Realm
                        }
                        
                        exit_x, exit_y = dungeon_portals.get(self.dungeon_id, (WINDOW_W // 2, WINDOW_H // 2))
...
# === TOWN RENDERING ===
        ...
        if self.dungeon_id == 0:
            ...
            if self.current_interior:
                # Black background
                self.canvas.create_rectangle(0, 0, WINDOW_W, WINDOW_H, fill='#0a0a0a', outline='')
                
                building = self.current_interior
                
                # Draw interior room (centered)
                room_x, room_y = WINDOW_W//2 - building['width']//2, WINDOW_H//2 - building['height']//2
                room_w, room_h = building['width'], building['height']
                
                # Floor
                self.canvas.create_rectangle(room_x, room_y, room_x + room_w, room_y + room_h,
                                            fill='#3a3a3a', outline='white', width=3)
                
                # Draw interior objects
                for obj in building.get('interior', []):
                    ox = room_x + obj['x']
                    oy = room_y + obj['y']
                    if obj['type'] == 'rect':
                        self.canvas.create_rectangle(ox, oy, ox + obj['w'], oy + obj['h'],
                                                    fill=obj['color'], outline='black', width=2)
                    elif obj['type'] == 'oval':
                        self.canvas.create_oval(ox, oy, ox + obj['w'], oy + obj['h'],
                                               fill=obj['color'], outline='black', width=2)
                
                # Draw player in interior
                self.canvas.create_oval(px-12, py-12, px+12, py+12, fill='white', outline='black', width=2)
                
                # Exit prompt
                self.canvas.create_text(WINDOW_W//2, WINDOW_H - 50, 
                                      text='Press DOWN to Exit', 
                                      fill='yellow', font=('Arial', 16, 'bold'))
                
                # Check for exit
                if self.keys.get('s') or self.keys.get('Down'):
                    self.current_interior = None
                    # Teleport player outside door
                    door_side = building.get('door_side', 'bottom')
                    if door_side == 'bottom':
                        self.player.y = building['y'] + building['height'] + 30
                    elif door_side == 'top':
                        self.player.y = building['y'] - 30
    ...
    # Continue with summons drawing...
        ...
        if self.dead:
            ...
            self.canvas.create_text(cw // 2 + 1, ch // 2 + 41,
                                    text="Non-soulbound items will be lost!",
...
class MainApp(tk.Tk):
    ...
    def start_game(self):
        try:
            ...
            self.geometry(f"{int(WINDOW_W + MAP_PANEL_W + 60)}x{int(WINDOW_H + 100)}")
