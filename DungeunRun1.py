...
class Player:
    ...
    def populate_skills(self):
        ...
        def fist_blast(player, game):
            ...
            game.spawn_projectile(player.x, player.y, ang, 6, 1.0, 8, 'brown', player.atk + 2)
        ...
        def fireball(player, game):
            ...
            target = min(game.room.enemies, key=lambda e: distance((player.x, player.y), (e.x, e.y)))
            ang = math.atan2(target.y - player.y, target.x - player.x)
            game.spawn_projectile(player.x, player.y, ang, 5, 2.0, 30, 'orange', player.mag*5)
        ...
        if self.class_name == 'Warrior':
            self.skills.append({'skill': ground_pound, 'name': 'Ground Pound', 'key': 2, 'level': 3, 'cooldown': 0, 'last_used': 0,  'cooldown_mod': 1.0})
            self.skills.append({'skill': fist_blast, 'name': 'Fist Blast', 'key': 3, 'level': 5, 'cooldown': 1.5, 'last_used': 0,  'cooldown_mod': 1.0})
            self.skills.append({'skill': strike, 'name': 'Rapid Strikes', 'key': 1, 'level': 1, 'cooldown': 0, 'last_used': 0})
        if self.class_name == 'Rogue':
            self.skills.append({'skill': dark_slash, 'name': 'Dark Slash', 'key': 1, 'level': 1, 'cooldown': 1.5, 'last_used': 0,  'cooldown_mod': 1.0})
...
# ---------- Projectile & Particle ----------
class Projectile:
    def __init__(self, x, y, angle, speed, life, radius, color, damage):
        self.x = x
        self.y = y
        self.angle = angle
        self.speed = speed
