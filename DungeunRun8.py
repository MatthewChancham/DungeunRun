    ...
    def populate_skills(self):
        ...
        def rapid_strike(player, game):
            ...
            def rapid_tick():
                ...
                game.spawn_particle(player.x, player.y, 30, 'yellow')
        ...
        def thorn_whip(player, game):
            ...
            # Leaves clustered around the tip angle for a thorny feel
            for i, d in enumerate((-0.3, 0.0, 0.3)):
                leaf = Particle(
                    ...
                    life=600,              # ms
                    rtype='leaf',
                    angle=angle_center + d,
                    radius=whip_radius - (i * 20)   # stagger leaves along branch
                )
        ...
        if self.class_name=='Mage':
            ...
            self.skills.append({'skill': mana_shield,'name':'Mana Shield','key':2,'level':1,'cooldown':2,'last_used':0,'cooldown_mod':1.0})
            self.skills.append({'skill': fireball,'name':'Fireball','key':3,'level':1,'cooldown':1.,'last_used':0,'cooldown_mod':1.0})
            ...
            self.skills.append({'skill': rapid_strike,'name':'Rapid Strike','key':3,'level':1,'cooldown':0,'last_used':0,'cooldown_mod':1.0})
            ...
            self.skills.append({'skill': thorn_whip,'name':'Nature Strike','key':1,'level':1,'cooldown':0.5,'last_used':0,'cooldown_mod':1.0})
            ...
            self.skills.append({'skill': chi_blast,'name':'Chi Blast','key':2,'level':5,'cooldown':1.5,'last_used':0,'cooldown_mod':1.0})
...
class Enemy:
    ...
    def update(self, game):
        ...
        prev_row, prev_col = self.room_row, self.room_col
        room_left   = self.room_col * ROOM_W
        room_right  = (self.room_col + 1) * ROOM_W
        room_top    = self.room_row * ROOM_H
        room_bottom = (self.room_row + 1) * ROOM_H

        self.x = clamp(self.x, room_left + self.size, room_right - self.size)
        self.y = clamp(self.y, room_top + self.size, room_bottom - self.size)        # Always dodge projectiles first
        self.dodge_projectiles(game)

        if self.role == "melee":
            if self.hp <= self.max_hp / 2:
                # Retreat if half health
                ...
                if d <= self.attack_range and now - self.last_attack >= self.attack_cooldown:
                    usable_skills = [
                        ...
                        if "melee" in sk.get("tags", []) and now - sk.get("last_used", 0) >= sk.get("cooldown", 1)
                    ]
                    if usable_skills:
                ...
                # If close enough, try to use a melee skill
                if d <= self.attack_range and now - self.last_attack >= self.attack_cooldown:
                    usable_skills = [
                        sk for sk in self.skills
                        if "melee" in sk.get("tags", []) and now - sk.get("last_used", 0) >= sk.get("cooldown", 1)
        ...
        elif self.role in ("ranged", "magic"):
            ...
            # Use skills
            usable_skills = [sk for sk in self.skills if now - sk.get("last_used", 0) >= sk.get("cooldown", 1)]
            if usable_skills:
                chosen = random.choice(usable_skills)
                chosen["skill"](self, game)
                chosen["last_used"] = now
...
def create_enemy_types_by_dungeon():
    return {
        1: [  # Dungeon 1: Forest
            lambda x, y: Enemy(
                "Swordman", 30, 5, 1.5, x, y, role="melee",
                skills=[
                    {"skill": enemy_dark_slash, "name": "Ark Slash", "tags": ["melee"], "cooldown": 1.5, "last_used": 0}
                ]
            ),
            lambda x, y: Enemy(
                "Spearman", 50, 5, 1.0, x, y, role="melee",
                skills=[
                    {"skill": enemy_strike, "name": "Strike", "tags": ["melee"], "cooldown": 0.5, "last_used": 0}
                ]
            ),
            lambda x, y: Enemy(
                "Archer", 35, 6, 3.0, x, y, role="ranged",
                skills=[
        ...
        2: [  # Dungeon 2: Volcano
            lambda x, y: Enemy(
                "Fire Imp", 35, 6, 2.0, x, y, role="melee",
                skills=[
                    {"skill": fire_slash, "name": "Fire Slash", "tags": ["melee"], "cooldown": 2.0, "last_used": 0}
                ]
            ),
            lambda x, y: Enemy(
            ...
            lambda x, y: Enemy(
                "Troll", 100, 12, 0.8, x, y, role="melee",
                skills=[
        ...
        4: [  # Dungeon 4: Shadow Realm
            lambda x, y: Enemy(
                ...
                skills=[
                    {"skill": dark_bolt, "name": "Dark Bolt", "tags": ["magic"], "cooldown": 0.5, "last_used": 0},
                    {"skill": summon_minion, "name": "Summon Minion", "tags": ["support"], "cooldown": 10.0, "last_used": 0}
                ]
            ),
            lambda x, y: Enemy(
                "Healer", 50, 8, 1.5, x, y, role="support",
                skills=[
                    {"skill": life_bolt, "name": "Life Bolt", "tags": ["support"], "cooldown": 2.0, "last_used": 0}
                ]
            ),
            lambda x, y: Enemy(
...
class Particle:
    def __init__(self, x, y, size, color, life=0.5, rtype='basic', angle=0.0, outline=False, radius=0, damage=0):
        self.x = x
        self.y = y
        self.size = size
        self.color = color
        ...
        self.rtype = rtype
        self.outline = outline   # 'basic' or 'blade'
        self.angle = angle
    ...
    def update(self, dt, game):
        ...
        if self.age >= self.life:
            return False
        if self.damage > 0 and distance((self.x, self.y), (game.player.x, game.player.y)) <= self.size:
            game.damage_player(self.damage)
        if self.rtype in ("branch", "leaf"):
            # Animate lash: extend then retract
            px, py = game.player.x, game.player.y
            progress = self.age / self.life

            # Extend for first half, retract for second half
            if progress < 0.5:
                reach = self.radius * (progress * 2)   # extend outward
            else:
                reach = self.radius * (2 - progress * 2)  # retract back

            # Swing angle slightly
...
class GameFrame(tk.Frame):
    ...
    def open_skill_page(self):
        ...
        # --- Bottom: All skills with assignment buttons ---
        ttk.Label(win, text="All Skills", font=("Arial", 14, "bold")).pack(pady=10)
        ...
        for sk in self.player.skills:
            ...
            ttk.Label(row, text=sk['name'], width=20).pack(side="left")

            # Add 5 buttons for slots 1–5
            for slot in range(1, 6):
                b = ttk.Button(row, text=str(slot),
    ...
    def spawn_projectile(self, x, y, angle, speed, life, radius, color, damage, owner="player", ptype='normal', stype="basic"):
        ...
        return proj
    def spawn_particle(self, x, y, size, color, life=0.5, rtype='basic'):
        # Create a new Particle with explicit keyword arguments
        particle = Particle(x, y, size, color, life=life, rtype=rtype)
