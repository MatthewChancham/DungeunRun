...
class GameFrame(tk.Frame):
    ...
    def draw_minimap(self):
        ...
        ROW_H    = 38
        LABEL_H  = 16 if all_display else 0
        ...
        if all_display:
            # Bottom of the buff area sits just above the hotbar
            area_bottom = ph - ITEM_H - 4
            label_y     = area_bottom - len(all_display) * ROW_H - 2
            mc.create_text(pw//2, label_y, text='BUFFS / DEBUFFS', fill='#aaaaaa',
                           font=('Arial', 7, 'bold'))
            for i, entry in enumerate(all_display):
                remaining = entry['remaining']
                # row 0 is nearest the hotbar, higher rows stack upward
                by = area_bottom - (i + 1) * ROW_H
                bg_fill = '#0d1a22' if entry['color'] == '#00eeff' else '#1a1a2a'
                mc.create_rectangle(4, by, pw-4, by+ROW_H-4,
                                    fill=bg_fill, outline='#333355', width=1)
                mc.create_text(16, by+ROW_H//2-4, text=entry['emoji'], font=('Arial', 13))
                mc.create_text(28, by+6, text=entry['name'][:14],
                               fill=entry['color'], font=('Arial', 7, 'bold'), anchor='w')
                mc.create_text(28, by+17, text=entry['desc'][:20],
                               fill='#aaaaaa', font=('Arial', 6), anchor='w')
                frac  = min(1.0, remaining / max(1, entry['duration']))
                bar_w = pw - 36
                mc.create_rectangle(28, by+26, 28+bar_w, by+32,
                                    fill='#333333', outline='')
                mc.create_rectangle(28, by+26, 28+int(bar_w*frac), by+32,
                                    fill=entry['bar_color'], outline='')
                mc.create_text(pw-6, by+6, text=f"{remaining:.1f}s",
                               fill=entry['color'], font=('Arial', 6), anchor='e')
