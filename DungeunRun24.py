...
class Player:
    ...
    # Immediate damage to enemies within the lash reach and arc
        ...
        def thousand_cuts(player, game):
            ...
            if distance((player.x, player.y), (target.x, target.y)) < 300:
                ...
                def spawn_slash():
                    ...
                    # Random angle each time → looks chaotic like real cuts
                ...
                # schedule the rest with random‑looking timing
                for i in range(1, num_slashes):
                    game.after(i * 40, spawn_slash)  # 40 ms apart → rapid flurry
        ...
        if self.class_name=='Mage':
            ...
            self.skills.append({'skill': mana_shield,'name':'Mana Bubble','key':0,'level':5,'cooldown':1,'last_used':0,'cooldown_mod':1.0})
...
class Enemy:
    ...
    def update(self, game):
        ...
        if player.hp <= 0:
            room_center_x = ROOM_W + ROOM_W / 2
            room_center_y = ROOM_H + ROOM_H / 2

            ang = math.atan2(room_center_y - self.y, room_center_x - self.x)
            self.x += math.cos(ang) * self.spd
            self.y += math.sin(ang) * self.spd
            ...
            return  # skip all combat/AI behavior
...
class Room:
    ...
    def create_town_layout(self):
        ...
        # === NPCs ===
        self.npcs.append(NPC("Eldrin", 400, 140, 'librarian', self.get_librarian_items()))
        self.npcs.append(NPC("Gorak", 650, 140, 'blacksmith', self.get_blacksmith_items()))
        self.npcs.append(NPC("Mystara", 140, 350, 'enchanter', self.get_enchanter_items()))
        self.npcs.append(NPC("Zephyr", 340, 310, 'alchemist', self.get_alchemist_items()))
        self.npcs.append(NPC("Berta", 550, 310, 'chef', self.get_chef_items()))
        self.npcs.append(NPC("Gemma", 160, 510, 'jeweler', self.get_jeweler_items()))
        self.npcs.append(NPC("Marcus", 400, 510, 'trader', SHOP_ITEMS[:10]))
        ...
        # === DECORATIONS ===
        # Street lamps
        lamp_positions = [(200, 200), (400, 200), (600, 200),
                         (200, 400), (600, 400)]
        ...
        # Trees around the edges (forest boundary)
        for i in range(20):
            # Top edge
            self.decorations.append({
                'type': 'tree',
        ...
        # Dungeon entrances
        dungeon_entrances = [
            {'x': 50, 'y': 250, 'id': 1, 'name': 'Forest Temple', 'color': '#4a7c59'},
            {'x': 750, 'y': 150, 'id': 2, 'name': 'Volcano Depths', 'color': '#c9302c'},
            {'x': 750, 'y': 400, 'id': 3, 'name': 'Ice Cavern', 'color': '#5bc0de'},
            {'x': 400, 'y': 550, 'id': 4, 'name': 'Shadow Realm', 'color': '#6f42c1'}
        ]
...
class GameFrame(tk.Frame):
    ...
    def update_camera(self):
        ...
        # Camera tries to center on player
        ...
        # For town, allow camera to move freely (no clamping)
        # The town is large enough that the camera can pan around
    ...
    def enter_dungeon(self, dungeon_id):
        ...
        print(f"Entered dungeon {dungeon_id}")
    ...
    def update_entities(self,dt):
        ...
        if self.dungeon_id == 0:
            ...
            self.nearby_npc = None
            for npc in self.room.npcs:
                if distance((self.player.x, self.player.y), (npc.x, npc.y)) < npc.interact_range:
                    self.nearby_npc = npc
                    break
            ...
            for deco in self.room.decorations:
                if deco['type'] == 'dungeon_entrance':
                    if distance((self.player.x, self.player.y), (deco['x'], deco['y'])) < 50:
    ...
    def update_player(self,dt):
        ...
        # Wall collision detection (ADD THIS after "player_moved = (p.x != old_x or p.y != old_y)")
        wall_thickness = 20
        opening_size = 150
        player_size = p.size
    ...
    # Continue with summons drawing...
            ...
            for deco in self.room.decorations:
                if deco['type'] == 'tree':
                    ...
                    size = deco['size']
                    # Tree trunk
                    self.canvas.create_rectangle(x-5, y-size, x+5, y+size, fill='#8B4513')
                    # Tree foliage
                    self.canvas.create_oval(x-size, y-size*2, x+size, y, fill='#228B22')
                
                elif deco['type'] == 'pond':
                    x = deco['x'] - cam_x
                    y = deco['y'] - cam_y
                    self.canvas.create_oval(
                        x, y, x + deco['width'], y + deco['height'],
                        fill='#4682B4', outline='#1E90FF', width=2
                    )
                    ...
                    y = deco['y'] - cam_y
                    # Lamp post
                    self.canvas.create_rectangle(x-3, y, x+3, y+40, fill='#4A4A4A')
                    # Lamp light
                    self.canvas.create_oval(x-8, y-15, x+8, y, fill='#FFD700', outline='#FFA500')
                    # Fire particles
                    for _ in range(3):
                        fx = x + random.uniform(-5, 5)
                        fy = y - 7 + random.uniform(-3, 3)
                        self.canvas.create_oval(fx-2, fy-2, fx+2, fy+2, fill='orange', outline='')
                    ...
                    self.canvas.create_oval(
                        ...
                        fill='', outline=deco['color'], width=5
                    )
                    self.canvas.create_oval(
                        x-size+5, y-size+5, x+size-5, y+size-5,
                        fill=deco['color'], stipple='gray50'
                    )
                    # Portal name
            ...
            for building in self.room.buildings:
                ...
                # Building body
                self.canvas.create_rectangle(x, y, x+w, y+h, fill=building['color'], outline='black', width=3)
                ...
                door_y = y + h - door_h
                self.canvas.create_rectangle(door_x, door_y, door_x+door_w, door_y+door_h, fill='#654321')
                
                # Windows
                window_size = 20
                for i in range(2):
                    wx = x + w//4 + i * w//2 - window_size//2
                    wy = y + h//3
                    self.canvas.create_rectangle(wx, wy, wx+window_size, wy+window_size, fill='#87CEEB', outline='black')
                
                # Building name
                self.canvas.create_text(x+w//2, y-roof_height-10, text=building['name'],
                                       fill='white', font=('Arial', 12, 'bold'))
        ...
        # === DUNGEON RENDERING ===
        # Draw walls with openings for adjacent rooms
            wall_thickness = 20
            opening_size = 150
            ...
            if self.room_row > 0:
                opening_x_start = WINDOW_W // 2 - opening_size // 2
                opening_x_end = opening_x_start + opening_size
                self.canvas.create_rectangle(0, 0, opening_x_start, wall_thickness, fill='#505050')
                self.canvas.create_rectangle(opening_x_end, 0, WINDOW_W, wall_thickness, fill='#505050')
            else:
            ...
            if self.room_row < ROOM_ROWS - 1:
                opening_x_start = WINDOW_W // 2 - opening_size // 2
                opening_x_end = opening_x_start + opening_size
                self.canvas.create_rectangle(0, WINDOW_H - wall_thickness, opening_x_start, WINDOW_H, fill='#505050')
                self.canvas.create_rectangle(opening_x_end, WINDOW_H - wall_thickness, WINDOW_W, WINDOW_H, fill='#505050')
            else:
            ...
            if self.room_col > 0:
                opening_y_start = WINDOW_H // 2 - opening_size // 2
                opening_y_end = opening_y_start + opening_size
                self.canvas.create_rectangle(0, 0, wall_thickness, opening_y_start, fill='#505050')
                self.canvas.create_rectangle(0, opening_y_end, wall_thickness, WINDOW_H, fill='#505050')
            else:
            ...
            if self.room_col < ROOM_COLS - 1:
                opening_y_start = WINDOW_H // 2 - opening_size // 2
                opening_y_end = opening_y_start + opening_size
                self.canvas.create_rectangle(WINDOW_W - wall_thickness, 0, WINDOW_W, opening_y_start, fill='#505050')
                self.canvas.create_rectangle(WINDOW_W - wall_thickness, opening_y_end, WINDOW_W, WINDOW_H, fill='#505050')
            else:
                self.canvas.create_rectangle(WINDOW_W - wall_thickness, 0, WINDOW_W, WINDOW_H, fill='#505050')
            
                # Set player coordinates without camera offset for dungeon
                px, py = self.player.x, self.player.y

    # === COMMON RENDERING (works for both town and dungeon) ===
    # From here, keep ALL your existing code for:
    ...
    # - Boss health bars
    # - HUD elements
    # (Everything from your original line ~2335 onwards)
        # Draw dark gray background
                self.canvas.create_rectangle(0, 0, WINDOW_W, WINDOW_H, fill='#2a2a2a', outline='')

                # Draw walls with openings for adjacent rooms
                wall_thickness = 20
                opening_size = 150  # Width/height of the opening

                # Top wall
                if self.room_row > 0:  # Has room above
                ...
                # Bottom wall
                if self.room_row < ROOM_ROWS - 1:  # Has room below
                    opening_x = WINDOW_W // 2 - opening_size // 2
                ...
                # Left wall
                if self.room_col > 0:  # Has room to the left
                ...
                # Right wall
                if self.room_col < ROOM_COLS - 1:  # Has room to the right
                    opening_y = WINDOW_H // 2 - opening_size // 2
