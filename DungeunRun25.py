...
class Room:
    ...
    def create_town_layout(self):
        ...
        # === BUILDINGS === (Make positions larger)
        # Player's house (top-left)
        self.buildings.append({
            'type': 'house',
            'x': 200, 'y': 200,  # Changed from 100, 100
        ...
        self.buildings.append({
            'type': 'library',
            'x': 600, 'y': 200,  # Changed from 350, 100
        ...
        self.buildings.append({
            'type': 'blacksmith',
            'x': 1000, 'y': 200,  # Changed from 600, 100
        ...
        self.buildings.append({
            'type': 'tower',
            'x': 200, 'y': 500,  # Changed from 100, 300
            'width': 80, 'height': 150,
            'color': '#9370DB',
        ...
        self.buildings.append({
            'type': 'shop',
            'x': 500, 'y': 480,  # Changed from 300, 280
            'width': 110, 'height': 90,
            'color': '#228B22',
            'name': "Alchemist"
        })
        ...
        self.buildings.append({
            'type': 'inn',
            'x': 850, 'y': 480,  # Changed from 500, 280
            'width': 140, 'height': 95,
            'color': '#CD853F',
            'name': "The Hearty Inn"
        })
        ...
        self.buildings.append({
            'type': 'shop',
            'x': 220, 'y': 780,  # Changed from 120, 480
            'width': 100, 'height': 85,
            'color': '#FF1493',
            'name': "Jeweler"
        })
        ...
        self.buildings.append({
            'type': 'shop',
            'x': 600, 'y': 780,  # Changed from 350, 480
            'width': 120, 'height': 90,
            'color': '#4169E1',
            'name': "Trader"
        })
        # === NPCs === (Update positions to match new building locations)
        ...
        # Add some villagers
        for i in range(8):  # More villagers
        ...
        # Dungeon entrances
        # Dungeon entrances (Update positions for larger map)
        dungeon_entrances = [
            {'x': 100, 'y': 400, 'id': 1, 'name': 'Forest Temple', 'color': '#4a7c59'},
            {'x': 1300, 'y': 250, 'id': 2, 'name': 'Volcano Depths', 'color': '#c9302c'},
            {'x': 1300, 'y': 650, 'id': 3, 'name': 'Ice Cavern', 'color': '#5bc0de'},
            {'x': 650, 'y': 1000, 'id': 4, 'name': 'Shadow Realm', 'color': '#6f42c1'}
        ]
...
class GameFrame(tk.Frame):
    ...
    def draw(self):
        ...
        if self.dungeon_id == 0:
            ...
            self.canvas.create_rectangle(0, 0, WINDOW_W, WINDOW_H, fill='#2d5016', outline='')
            
            # Draw roads (horizontal and vertical streets)
            road_color = '#4a4a4a'
            road_width = 80
            
            # Horizontal roads (convert world to screen coordinates)
            for road_y in [250, 550, 850]:
                screen_y = road_y - self.camera_y
                self.canvas.create_rectangle(
                    0, screen_y - road_width//2,
                    WINDOW_W, screen_y + road_width//2,
                    fill=road_color, outline=''
                )
            
            # Vertical roads
            for road_x in [300, 700]:
                screen_x = road_x - self.camera_x
                self.canvas.create_rectangle(
                    screen_x - road_width//2, 0,
                    screen_x + road_width//2, WINDOW_H,
                    fill=road_color, outline=''
                )
