...
SKILL_TREES = {
    'Warrior': [
        ...
        {'name': 'Ground Pound',            'tier': 2, 'prereq': ['Strikes'],               'cost': 1, 'type': 'active',  'desc': 'Slam the ground — AoE damage + knockback to all nearby foes.', 'branch': 'left',   'passive': None},
        {'name': 'Fist Blast',              'tier': 3, 'prereq': ['Ground Pound'],          'cost': 1, 'type': 'active',  'desc': 'A powerful forward fist blast dealing heavy damage.',           'branch': 'left',   'passive': None},
        {'name': 'Lingering Aura of Valour','tier': 4, 'prereq': ['Fist Blast'],            'cost': 2, 'type': 'active',  'desc': 'Radiate a damaging aura that also blocks enemy projectiles.',   'branch': 'left',   'passive': None},
    ...
    'Mage': [
        ...
        {'name': 'Fireball',                'tier': 2, 'prereq': ['Mana Bolt'],             'cost': 1, 'type': 'active',  'desc': 'Explosive fireball — deals AoE fire damage on impact.',         'branch': 'left',   'passive': None},
        {'name': 'Icicle',                  'tier': 3, 'prereq': ['Fireball'],              'cost': 1, 'type': 'active',  'desc': 'Ice spike that shatters on impact with frost AoE.',             'branch': 'left',   'passive': None},
        {'name': 'Chain Lightning',         'tier': 4, 'prereq': ['Icicle'],                'cost': 2, 'type': 'active',  'desc': 'Lightning bolt that chains between up to 5 enemies.',           'branch': 'left',   'passive': None},
        {'name': 'Arcane Mind',             'tier': 2, 'prereq': ['Mana Bolt'],             'cost': 1, 'type': 'passive', 'desc': 'PASSIVE: +15 Max Mana, +2 Intelligence.',                       'branch': 'right',  'passive': None},
        {'name': 'Mana Bubble',             'tier': 3, 'prereq': ['Arcane Mind'],           'cost': 1, 'type': 'active',  'desc': 'Toggle a mana-powered shield that repels enemies and projectiles.', 'branch': 'right', 'passive': None},
        {'name': 'Mana Surge',              'tier': 4, 'prereq': ['Mana Bubble'],           'cost': 2, 'type': 'passive', 'desc': 'PASSIVE: +4 Intelligence, +4 Wisdom, +2 Mana Regen.',           'branch': 'right',  'passive': None},
    ],
    'Rogue': [
        ...
        {'name': 'Shadow Dagger',           'tier': 2, 'prereq': ['Dark Slash'],            'cost': 1, 'type': 'active',  'desc': 'Hurl a shadowy dagger at high speed.',                          'branch': 'left',   'passive': None},
        {'name': 'Blink',                   'tier': 3, 'prereq': ['Shadow Dagger'],         'cost': 1, 'type': 'active',  'desc': 'Teleport a great distance toward the nearest enemy.',           'branch': 'left',   'passive': None},
        {'name': 'Thousand Cuts',           'tier': 4, 'prereq': ['Blink'],                 'cost': 2, 'type': 'active',  'desc': 'Unleash a rapid flurry of slashes on a single target.',         'branch': 'left',   'passive': None},
    ...
    'Cleric': [
        ...
        {'name': 'Minor Heal',              'tier': 2, 'prereq': ['Light Bolt'],            'cost': 1, 'type': 'active',  'desc': 'Restore HP equal to your magic power.',                         'branch': 'left',   'passive': None},
        {'name': 'Light Beam',              'tier': 3, 'prereq': ['Minor Heal'],            'cost': 1, 'type': 'active',  'desc': 'Summon a rotating beam of holy light for 3 seconds.',           'branch': 'left',   'passive': None},
        {'name': 'Summon Range Sentry',     'tier': 4, 'prereq': ['Light Beam'],            'cost': 2, 'type': 'active',  'desc': 'Summon a sentry that fires holy bolts at all enemies.',         'branch': 'left',   'passive': None},
    ...
    'Druid': [
        ...
        {'name': 'Summon Wolf',             'tier': 2, 'prereq': ['Thorn Whip'],            'cost': 1, 'type': 'active',  'desc': 'Summon a loyal wolf that attacks enemies for you.',              'branch': 'left',   'passive': None},
        ...
        {'name': 'Lashing Vines',           'tier': 4, 'prereq': ['Leaf Shot'],             'cost': 2, 'type': 'active',  'desc': 'Erupt vines in all directions, lashing nearby foes.',           'branch': 'left',   'passive': None},
    ...
    'Monk': [
        ...
        {'name': 'Chi Blast',               'tier': 2, 'prereq': ['Chi Strike'],            'cost': 1, 'type': 'active',  'desc': 'Blast chi energy forward in a focused burst.',                  'branch': 'left',   'passive': None},
    ...
    'Ranger': [
        ...
        {'name': 'Multishot',               'tier': 2, 'prereq': ['Arrow Shot'],            'cost': 1, 'type': 'active',  'desc': 'Fire arrows at up to 3 enemies simultaneously.',                'branch': 'left',   'passive': None},
        {'name': 'Fire Trap',               'tier': 3, 'prereq': ['Multishot'],             'cost': 1, 'type': 'active',  'desc': 'Place a fire trap that ignites enemies on contact.',            'branch': 'left',   'passive': None},
        {'name': 'Frost Trap',              'tier': 4, 'prereq': ['Fire Trap'],             'cost': 2, 'type': 'active',  'desc': 'Place a frost trap that freezes and slows enemies.',            'branch': 'left',   'passive': None},
...
class Player:
    ...
    def update_stats(self):
        ...
        # Clamp shield: if armour newly equipped, fill to full; else keep current
    ...
    def populate_skills(self):
        ...
        def chi_blast(player, game):
            ...
            def spawn_bolt():
                _mx, _my = game.get_mouse_world_pos()
                ang = math.atan2(_my - player.y, _mx - player.x)
                game.spawn_projectile(player.x, player.y, ang, 11, 3, 8, 'cyan', player.vit*2, owner='player', stype='bolt')
        ...
        def speed_boost(player, game):
            ...
            correct_base_speed = 2 + player.agility * 0.3
            ...
            def reset_speed():
                ...
                player.base_speed = 2 + player.agility * 0.3
                player.speed = player.base_speed
        ...
        if self.class_name=='Mage':
            ...
            self.skills.append({'skill': fist_blast,'name':'Fist Blast','key':0,'level':1,'cooldown':1,'last_used':0,'cooldown_mod':1.0})
    ...
    def upgrade_soulbound_items(self):
        ...
        evolutions = {
            'Warrior': {
                10: {'name': 'Iron Spear', 'stats': {'strength': 3, 'vitality': 3}, 'skills': ['Flame Strike']},
                25: {'name': 'Spear of Valour', 'stats': {'strength': 6, 'vitality': 6}, 'skills': ['Flame Strike', 'Life Drain']},
...
class ConsumableItem:
    ...
    def __init__(self, name, subtype, rarity="Common", price=0,
                 hp_restore=0, mana_restore=0, speed_boost=0,
                 atk_boost=0, boost_duration=0, description=""):
        ...
        self.mana_restore=mana_restore; self.speed_boost=speed_boost
        self.atk_boost=atk_boost; self.boost_duration=boost_duration
        self.description=description; self.soulbound=False; self.skills=[]; self.stats={}
        self.count = 1
    ...
    def get_description(self):
        ...
        if self.speed_boost:  parts.append(f"+{int(self.speed_boost*100)}% Speed ({self.boost_duration}s)")
        if self.atk_boost:    parts.append(f"+{self.atk_boost} ATK ({self.boost_duration}s)")
    ...
    def use(self, player):
        ...
        if self.speed_boost and self.boost_duration:
            player._speed_boost_end = time.time() + self.boost_duration
            player._speed_boost = self.speed_boost
        if self.atk_boost and self.boost_duration:
            player._atk_boost_end = time.time() + self.boost_duration
            player._atk_boost_val = self.atk_boost
        return True
    ...
    def to_dict(self):
        return {"consumable":True,"name":self.name,"item_type":self.item_type,
                ...
                "speed_boost":self.speed_boost,"atk_boost":self.atk_boost,
    ...
    def from_dict(cls,data):
        obj = cls(name=data["name"],subtype=data.get("subtype","health_potion"),
                   ...
                   speed_boost=data.get("speed_boost",0),atk_boost=data.get("atk_boost",0),
...
CONSUMABLE_SHOP_ITEMS = [
    ...
    ConsumableItem("Elixir of Power",    "elixir",       "Rare",    price=300,
                   hp_restore=200,mana_restore=200,atk_boost=5,boost_duration=30),
    ...
    ConsumableItem("Roast Meat","meat", "Uncommon",price=10, hp_restore=50,
                   speed_boost=0.1,boost_duration=20,description="+HP and brief speed boost."),
    ConsumableItem("Hero Stew", "stew", "Rare",    price=150,
                   hp_restore=200,atk_boost=8,boost_duration=60,description="Full meal buff."),
]
...
def ice_blast(enemy, game):
    ...
    if distance((enemy.x, enemy.y), (game.player.x, game.player.y)) <= radius:
        ...
        game.player.speed == 0   # reduce speed by 30%
...
class Boss(Enemy):
    ...
    def freeze_aura(self, game):
        ...
        for _ in range(20):
            x = self.x + random.uniform(-120,120)
            y = self.y + random.uniform(-120,120)
            game.spawn_particle(x, y, random.uniform(5,10), 'cyan', rtype="frost")
        if distance((self.x, self.y), (game.player.x, game.player.y)) < 80:
            game.player.speed *= 0.5
            game.spawn_particle(game.player.x, game.player.y, 15, 'blue', rtype="frost", owner="enemy")
    def heal(enemy, game):
        """Heals the enemy with a visual particle effect."""
...
class Room:
    ...
    def create_town_layout(self):
        ...
        self.buildings.append({
            ...
            'indoor_spawn_x': WINDOW_W * 3 // 4,  # spawn on the right (open shop floor)
        ...
        self.npcs.append(indoor_npc("Marcus",  660, 825, 'trader',     SHOP_ITEMS[:10]))
...
class GameFrame(tk.Frame):
    def __init__(self,parent,player,on_quit_to_menu,dungeon_id=1):
        ...
        self.map_canvas = tk.Canvas(self, bg='#0a0a0a', highlightthickness=0)
        ...
        # Item hotbar (3 slots for consumables, T/Y/U)
        self.hotbar_items = [None, None, None]   # index 0,1,2 = T,Y,U
    ...
    def _get_interior_layout(self, building):
        """Return (walls, objects) for a building. walls block movement; objects with
        'collision':(x1,y1,x2,y2) also block. All coords in 0..WINDOW_W/H space."""
        ...
        def lbl(text, x, y, color='#888888'):
            return {'type':'label','x':x,'y':y,'text':text,'color':color}
        def shelf(x,y,w,h=12):
            return {'type':'shelf','x':x,'y':y,'w':w,'h':h,'collision':(x,y,x+w,y+h)}
        def col_box(x,y,w,h):
            return {'type':'collision_only','collision':(x,y,x+w,y+h)}
        def rect_col(x,y,w,h,color):
            return {'type':'rect','x':x,'y':y,'w':w,'h':h,'color':color,
                    'collision':(x,y,x+w,y+h)}
        ...
        if btype == 'house':
            ...
            for txt,x,y in [('BEDROOM',midX//2+ow,ow+14),('STORAGE',midX+(W-ow-midX)//2,ow+14),
                             ('KITCHEN',midX//2+ow,midY+iw+14),('LIVING ROOM',midX+(W-ow-midX)//2,midY+iw+14)]:
                objs.append(lbl(txt,x,y,'#888888'))
            # BEDROOM
            objs += [{'type':'bed','x':ow+30,'y':ow+40,'w':180,'h':100,
                      'collision':(ow+30,ow+40,ow+210,ow+140)},
                     rect_col(ow+225,ow+55,44,44,'#5c3d1e'),
                     {'type':'candle','x':ow+247,'y':ow+53},
                     {'type':'window','x':ow+30,'y':ow+2,'w':90,'h':20},
                     {'type':'rug','x':ow+30,'y':ow+155,'w':220,'h':100,'color':'#8B2222'},
                     rect_col(ow+265,ow+30,65,160,'#5c3d1e')]
            # STORAGE
            objs.append({'type':'chest','x':midX+30,'y':ow+45,'w':80,'h':60,
                         'collision':(midX+30,ow+45,midX+110,ow+105)})
            for si in range(4):
                objs.append(shelf(midX+140,ow+25+si*52,W-ow-midX-155))
            # KITCHEN
            ky = midY+iw
            objs += [{'type':'stove','x':ow+25,'y':ky+30,'w':100,'h':90,
                      'collision':(ow+25,ky+30,ow+125,ky+120)},
                     {'type':'pot','x':ow+75,'y':ky+18},
                     rect_col(ow+135,ky+30,180,60,'#8B5e3c'),
                     {'type':'table','x':ow+30,'y':ky+180,'w':140,'h':80,
                      'collision':(ow+30,ky+180,ow+170,ky+260)},
                     {'type':'chair','x':ow+40,'y':ky+265,'w':38,'h':38,
                      'collision':(ow+40,ky+265,ow+78,ky+303)},
                     {'type':'chair','x':ow+100,'y':ky+265,'w':38,'h':38,
                      'collision':(ow+100,ky+265,ow+138,ky+303)}]
            # LIVING ROOM
            lx,ly = midX,midY+iw
            objs += [{'type':'fireplace','x':W-ow-130,'y':ly+20,'w':115,'h':95,
                      'collision':(W-ow-130,ly+20,W-ow-15,ly+115)},
                     {'type':'rug','x':lx+30,'y':ly+90,'w':270,'h':170,'color':'#1a3a6a'},
                     {'type':'couch','x':lx+30,'y':ly+120,'w':250,'h':70,
                      'collision':(lx+30,ly+92,lx+280,ly+195)},
                     rect_col(lx+135,ly+220,80,50,'#5c3d1e'),
                     {'type':'window','x':lx+20,'y':ly+22,'w':80,'h':20},
                     {'type':'candle','x':lx+175,'y':ly+214}]

        # ── LIBRARY ────────────────────────────────────────────────────────
        elif btype == 'library':
            BC = ['#cc4444','#4444cc','#44cc44','#cccc44','#cc44cc',
                  '#44cccc','#cc8844','#8844cc','#884444','#448844']
            # Aisle bookshelves: vertical units running top-to-bottom across the room
            aisle_w = 22    # shelf unit width
            gap_w   = 85    # walkable aisle between units
            # Left wall shelf
            objs.append(rect_col(ow, ow, aisle_w, H-ow*2, '#5c3d1e'))
            for bi in range((H-ow*2)//28):
                objs.append({'type':'book','x':ow+2,'y':ow+4+bi*28,'w':aisle_w-4,'h':25,
                             'color':BC[bi%len(BC)]})
            # Right wall shelf
            objs.append(rect_col(W-ow-aisle_w, ow, aisle_w, H-ow*2, '#5c3d1e'))
            for bi in range((H-ow*2)//28):
                objs.append({'type':'book','x':W-ow-aisle_w+2,'y':ow+4+bi*28,'w':aisle_w-4,'h':25,
                             'color':BC[(bi+4)%len(BC)]})
            # Three free-standing aisle shelf units (double-sided bookcases)
            for ai, ax in enumerate([ow+aisle_w+gap_w, ow+aisle_w+gap_w*2+aisle_w,
                                      ow+aisle_w+gap_w*3+aisle_w*2]):
                objs.append(rect_col(ax, ow, aisle_w, H-ow*2-dg-10, '#5c3d1e'))
                for bi2 in range((H-ow*2-dg-10)//28):
                    objs.append({'type':'book','x':ax+2,'y':ow+4+bi2*28,'w':aisle_w-4,'h':25,
                                 'color':BC[(bi2+ai*3)%len(BC)]})
            # Red carpet down the main centre aisle
            centre_ax = ow + aisle_w + gap_w*2 + aisle_w + aisle_w//2 + gap_w//2
            carpet_x  = centre_ax - 28
            objs.append({'type':'carpet','x':carpet_x,'y':ow+2,'w':56,'h':H-ow*2-4,
                         'color':'#8B0000','outline_color':'#FFD700'})
            # Reading desks along the carpet
            for di in range(4):
                dy = ow+40 + di*(H-ow*2-80)//4
                objs += [{'type':'desk','x':carpet_x-75,'y':dy,'w':65,'h':44,
                           'collision':(carpet_x-75,dy,carpet_x-10,dy+44)},
                         {'type':'desk','x':carpet_x+10,'y':dy,'w':65,'h':44,
                           'collision':(carpet_x+10,dy,carpet_x+75,dy+44)},
                         {'type':'candle','x':carpet_x-43,'y':dy-2},
                         {'type':'candle','x':carpet_x+43,'y':dy-2}]
            # Hanging lanterns over centre aisle
            for ly2 in [H//5,H*2//5,H*3//5,H*4//5]:
                objs.append({'type':'lantern','x':centre_ax,'y':ly2})
            objs.append(lbl('LIBRARY', W//2, ow//2+4, '#FFD700'))

        # ── BLACKSMITH ─────────────────────────────────────────────────────
        elif btype == 'blacksmith':
            # No full-height divider — forge area is left, open shop is right.
            # A low counter wall separates them so player can walk freely.
            forge_right = W * 5 // 12    # right edge of forge alcove
            # Low divider (waist-height counter, only partial) so player never gets stuck
            walls += [(forge_right, H*3//5, forge_right+iw, H-ow-dg//2-10),
                      (forge_right, H-ow-dg//2+70, forge_right+iw, H-ow)]

            # FORGE area (left)
            objs += [{'type':'forge','x':ow+15,'y':ow+20,'w':forge_right-ow-30,'h':H//2-20,
                      'collision':(ow+15,ow+20,forge_right-ow-30,H//2+30)},
                     {'type':'anvil','x':ow+20,'y':H*2//3-10,
                      'collision':(ow+10,H*2//3-20,ow+100,H*2//3+30)},
                     # Water trough
                     rect_col(ow+120,H*2//3-10,80,30,'#1a3a4a')]

            # Weapons scattered on the forge floor and walls — all DRAWN not emoji
            # Wall rack (top of forge side)
            objs.append(rect_col(ow,ow+4,forge_right-ow-15,18,'#4a3020'))  # rack board
            weapon_types = ['sword','sword','axe','spear','sword','axe']
            spacing = (forge_right-ow-30) // len(weapon_types)
            for wi2, wt in enumerate(weapon_types):
                wx2 = ow + 12 + wi2*spacing
                objs.append({'type':'wall_weapon','x':wx2,'y':ow+10,'weapon':wt,'angle':-0.3})

            # Weapons on the floor/ground (leaning or laid)
            floor_weapons = [('sword', ow+30, H*3//4, 0.2),
                             ('axe',   ow+80, H*3//4+20, -0.15),
                             ('spear', ow+140,H*3//4-10, 0.05)]
            for wt,wx3,wy3,wa in floor_weapons:
                objs.append({'type':'floor_weapon','x':wx3,'y':wy3,'weapon':wt,'angle':wa})

            # SHOP area (right) — display + counter
            objs.append(rect_col(forge_right+iw+15, ow+20, W-ow-forge_right-iw-30, 70, '#4a3020'))
            # Weapon display shelf on back wall
            for wi3 in range(4):
                wx4 = forge_right+iw+25 + wi3*((W-ow-forge_right-iw-40)//4)
                objs.append({'type':'wall_weapon','x':wx4,'y':ow+30,'weapon':['sword','axe','spear','sword'][wi3],'angle':0})
            # Armour display (shields + helmet drawn as shapes)
            for si2 in range(3):
                sx2 = forge_right+iw+20 + si2*80
                objs.append({'type':'drawn_shield','x':sx2,'y':ow+120})
                objs.append(col_box(sx2-20,ow+100,40,50))
            # Sales counter at bottom
            objs.append({'type':'display_counter','x':forge_right+iw+15,'y':H*3//5+20,
                         'w':W-ow-forge_right-iw-30,'h':55,
                         'collision':(forge_right+iw+15,H*3//5+20,W-ow-15,H*3//5+75)})
            objs.append(lbl('FORGE',forge_right//2+ow,ow+8,'#888888'))
            objs.append(lbl('SHOP',forge_right+iw+(W-ow-forge_right-iw)//2,ow+8,'#888888'))

        # ── ENCHANTER TOWER ────────────────────────────────────────────────
        elif btype == 'tower':
            # Open room — just corner pillars and bookshelf segments that make sense
            p_sz = 28
            for px4,py4 in [(ow+20,ow+20),(W-ow-p_sz-20,ow+20),
                            (ow+20,H-ow-p_sz-20),(W-ow-p_sz-20,H-ow-p_sz-20)]:
                objs += [rect_col(px4,py4,p_sz,p_sz,'#3a1a6a')]
            # Crystal ball stand only (right side, no clutter)
            objs += [rect_col(W-ow-70,ow+50,50,60,'#3a1a6a'),
                     {'type':'candle','x':W-ow-45,'y':ow+48},
                     {'type':'candle','x':W-ow-45,'y':H-ow-50}]
            # Small desk for spellbook (left side)
            objs += [{'type':'desk','x':ow+30,'y':H//2-25,'w':90,'h':50,
                      'collision':(ow+30,H//2-25,ow+120,H//2+25)},
                     {'type':'candle','x':ow+75,'y':H//2-28}]
            objs.append(lbl("ENCHANTER'S TOWER",W//2,ow//2+4,'#aa44ff'))

        # ── ALCHEMIST ──────────────────────────────────────────────────────
        elif btype == 'shop' and building.get('indoor_npc_name') == 'Zephyr':
            objs.append(lbl("ALCHEMIST'S LAB",W//2,ow//2+4,'#44ff44'))
            # Three cauldrons of different sizes — centre-left
            cauldron_data = [
                (ow+60,  H//2-40, 55),   # large
                (ow+160, H//2-20, 38),   # medium
                (ow+240, H//2-5,  26),   # small
            ]
            for cx4,cy4,cr in cauldron_data:
                objs += [{'type':'big_cauldron','x':cx4,'y':cy4,'r':cr},
                         col_box(cx4-cr-10,cy4-cr//2,cr*2+20,cr+50)]
            # Ingredient storage: thick crates near the centre, not at the edges
            for ci,col2 in enumerate(['#3a5a2a','#5a3a1a','#1a3a5a']):
                cx5 = W//2+80 + ci*95
                cy5 = H//2 - 40
                objs += [rect_col(cx5,cy5,80,100,col2)]
                # Potion bottles on top of crate
                for pi in range(3):
                    objs.append({'type':'potion','x':cx5+15+pi*25,'y':cy5-18,
                                 'color':['#ff4444','#4488ff','#44ff44'][pi]})
            # Worktable top wall
            objs += [rect_col(ow+20,ow+20,W-ow*2-40,55,'#8B5e3c'),
                     {'type':'potion','x':ow+50,'y':ow+10,'color':'#ff4444'},
                     {'type':'potion','x':ow+90,'y':ow+10,'color':'#4488ff'},
                     {'type':'potion','x':ow+130,'y':ow+10,'color':'#44ff44'},
                     {'type':'potion','x':ow+170,'y':ow+10,'color':'#ffff44'},
                     {'type':'potion','x':ow+210,'y':ow+10,'color':'#ff44ff'},
                     {'type':'candle','x':W-ow-60,'y':ow+18}]
            # Bookshelf of recipes (right wall)
            objs.append(rect_col(W-ow-26,ow+90,26,H-ow*2-90,'#5c3d1e'))
            BC2 = ['#cc4444','#4444cc','#44cc44','#cccc44']
            for bi3 in range((H-ow*2-90)//28):
                objs.append({'type':'book','x':W-ow-24,'y':ow+90+4+bi3*28,'w':22,'h':25,
                             'color':BC2[bi3%4]})

        # ── JEWELER ────────────────────────────────────────────────────────
        elif btype == 'shop' and building.get('indoor_npc_name') == 'Gemma':
            objs.append(lbl("JEWELER'S SHOP",W//2,ow//2+4,'#ff44ff'))
            # Display case — narrower, shifted left so right side is accessible
            case_w = W//2 + 40
            case_x = ow+20
            case_y = H//2-30
            objs += [{'type':'display_counter','x':case_x,'y':case_y,'w':case_w,'h':60,
                      'collision':(case_x,case_y,case_x+case_w,case_y+60)}]
            gem_data = [('#ff4444','Ruby'),('#4444ff','Sapphire'),('#44ff44','Emerald'),
                        ('#ff44ff','Amethyst'),('#ffff44','Topaz'),('#44ffff','Aqua')]
            gem_spacing = (case_w-30)//len(gem_data)
            for gi,(gc,gn) in enumerate(gem_data):
                gx2 = case_x+15 + gi*gem_spacing
                objs.append({'type':'gem_display','x':gx2,'y':case_y-18,'color':gc,'name':gn})
            # Workbench top-left
            objs += [rect_col(ow+20,ow+25,180,65,'#5c3d1e'),
                     {'type':'candle','x':ow+100,'y':ow+22}]
            # Small shelves with gems (top-right)
            for ri in range(3):
                ry2 = ow+30+ri*55
                rx2 = W-ow-120
                objs.append(rect_col(rx2,ry2,100,12,'#8B5e3c'))
                for ji in range(4):
                    objs.append({'type':'gem_display','x':rx2+12+ji*24,'y':ry2-14,
                                 'color':gem_data[(ri*4+ji)%6][0],'name':''})
            # Hanging lanterns
            for lx3 in [W//4,W*3//4]:
                objs.append({'type':'lantern','x':lx3,'y':ow+55})

        # ── GENERAL TRADER ─────────────────────────────────────────────────
        elif btype == 'shop' and building.get('indoor_npc_name') == 'Marcus':
            objs.append(lbl('GENERAL STORE',W//2,ow//2+4,'#4488ff'))
            # Large central display island (3 counters in a U-shape)
            island_x,island_y = W//2-130,H//2-40
            # Left side of U
            objs += [rect_col(island_x,island_y,45,140,'#8B5e3c'),
                     rect_col(island_x+45,island_y,140,45,'#8B5e3c'),   # top
                     rect_col(island_x+45,island_y+95,140,45,'#8B5e3c')]# bottom
            # Weapon rack along left wall
            objs.append(rect_col(ow,ow,22,H-ow*2,'#4a3020'))
            for wi4 in range(4):
                wy4 = ow+30+wi4*((H-ow*2-60)//4)
                objs.append({'type':'wall_weapon','x':ow+11,'y':wy4,
                             'weapon':['sword','axe','spear','sword'][wi4],'angle':0.05})
            # Shelves along top wall
            objs.append(rect_col(ow+30,ow,W-ow*2-30,24,'#5c3d1e'))
            BC3=['#cc4444','#4444cc','#44cc44','#cccc44','#cc44cc','#44cccc']
            for bi4 in range((W-ow*2-50)//26):
                objs.append({'type':'book','x':ow+35+bi4*26,'y':ow+3,'w':23,'h':20,
                             'color':BC3[bi4%len(BC3)]})
            # Shelving unit right wall
            objs.append(rect_col(W-ow-26,ow+30,26,H-ow*2-30,'#5c3d1e'))
            for bi5 in range((H-ow*2-50)//40):
                objs.append(shelf(W-ow-26,ow+35+bi5*40,26,10))
            # Sales desk bottom-right
            objs += [rect_col(W-ow-160,H-ow-70,140,60,'#8B5e3c'),
                     {'type':'candle','x':W-ow-90,'y':H-ow-75}]
            # Barrel cluster bottom-left
            for bri in range(3):
                objs += [{'type':'barrel','x':ow+35+bri*55,'y':H-ow-60,
                          'collision':(ow+35+bri*55-26,H-ow-90,ow+35+bri*55+26,H-ow-20)}]
            # Crate stack near left
            objs += [rect_col(ow+30,H*2//3,60,80,'#6B3A1F'),
                     rect_col(ow+30,H*2//3-50,60,45,'#8B5e3c')]

        # ── BAKERY ─────────────────────────────────────────────────────────
        elif btype == 'inn':
            divY = int(H*0.42)
            divX = int(W*0.58)
            door_x_gap = int(W*0.22)
            walls += [(ow,divY,door_x_gap-dg//2,divY+iw),
                      (door_x_gap+dg//2,divY,W-ow,divY+iw),
                      (divX,divY+iw,divX+iw,H//2+H//8-dg//2),
                      (divX,H//2+H//8+dg//2,divX+iw,H-ow)]
            for txt,x,y in [('KITCHEN',W//2,ow+14),
                             ('COUNTER',(ow+divX)//2,divY+iw+14),
                             ('STORAGE',divX+(W-ow-divX)//2,divY+iw+14)]:
                objs.append(lbl(txt,x,y,'#888888'))

            # KITCHEN — wide oven (left, not too tall)
            oven_h = divY - ow - 30
            objs += [{'type':'oven','x':ow+15,'y':ow+25,'w':W//3,'h':oven_h,
                      'collision':(ow+15,ow+25,ow+15+W//3,ow+25+oven_h)},
                     {'type':'pot','x':ow+15+W//6,'y':ow+15}]
            # Prep bench (right side of kitchen)
            bench_x = ow+15+W//3+20
            objs += [rect_col(bench_x,ow+25,W-ow-bench_x-ow,55,'#8B5e3c'),
                     # Items on bench (drawn shapes, not emoji)
                     {'type':'bread_loaf','x':bench_x+30,'y':ow+12},
                     {'type':'bread_loaf','x':bench_x+90,'y':ow+12},
                     {'type':'bread_loaf','x':bench_x+150,'y':ow+12}]
            objs.append(shelf(bench_x,ow+90,W-ow-bench_x-ow,12))
            objs.append({'type':'candle','x':bench_x+20,'y':ow+86})

            # COUNTER (bottom-left) — narrow enough to allow access
            counter_w = divX - ow - 55
            objs += [{'type':'display_counter','x':ow+20,'y':divY+iw+45,
                      'w':counter_w,'h':50,
                      'collision':(ow+20,divY+iw+45,ow+20+counter_w,divY+iw+95)},
                     {'type':'bread_loaf','x':ow+50,'y':divY+iw+30},
                     {'type':'bread_loaf','x':ow+110,'y':divY+iw+30},
                     {'type':'candle','x':ow+170,'y':divY+iw+40}]
            # Seating
            for ti,(tx2,ty2) in enumerate([(ow+30,divY+iw+150),(ow+140,divY+iw+150)]):
                objs += [{'type':'table','x':tx2,'y':ty2,'w':80,'h':55,
                          'collision':(tx2,ty2,tx2+80,ty2+55)},
                         {'type':'chair','x':tx2+10,'y':ty2+60,'w':32,'h':32,
                          'collision':(tx2+10,ty2+60,tx2+42,ty2+92)}]

            # STORAGE (bottom-right)
            for bi6,(bx3,lbl2) in enumerate([(divX+iw+25,'Honey'),
                                              (divX+iw+95,'Salt'),
                                              (divX+iw+165,'Milk')]):
                objs += [{'type':'barrel','x':bx3,'y':divY+iw+60,
                          'collision':(bx3-26,divY+iw+30,bx3+26,divY+iw+90)},
                         lbl(lbl2,bx3,divY+iw+98,'#ccbb99')]
            objs.append(shelf(divX+iw+15,divY+iw+125,W-ow-divX-iw-20,14))
            for si3 in range(2):
                objs += [{'type':'sack','x':divX+iw+25+si3*95,'y':H-ow-75,
                          'collision':(divX+iw-3+si3*95,H-ow-105,divX+iw+53+si3*95,H-ow-20)}]
    ...
    def draw_hotbar(self):
        """Draw skill hotbar (top-left) and item hotbar (bottom-centre, slightly clipped)."""
        p = self.player
        now = time.time()

        # ── SKILL hotbar (top-left, slots 1-5) ───────────────────────────────
        slot_size = 44
        gap = 6
        start_x = 10
        y_skill = 80
        ...
        for i in range(1, 6):
            ...
            bg = '#555555' if not selected else '#dddddd'
            ...
            if sk:
                ...
                if cd > 0 and elapsed < cd:
                    ...
                    self.canvas.create_rectangle(x, y_skill, x + slot_size, y_skill + overlay_h,
        ...
        # ── ITEM hotbar — outline-only frames, fully see-through ─────────────
        item_slot_size = 44
        item_gap = 8
        labels = ['T', 'Y', 'U']
        total_w = 3 * item_slot_size + 2 * item_gap
        ix0 = (WINDOW_W - total_w) // 2
        iy = WINDOW_H - item_slot_size - 4   # sits just above bottom edge

        for i in range(3):
            x = ix0 + i * (item_slot_size + item_gap)
            selected = (i == self.active_item_slot)

            # Outline only — no fill so the game shows through completely
            outline_col = '#ffffff' if selected else '#8888bb'
            lw = 2 if selected else 1
            self.canvas.create_rectangle(
                x, iy, x + item_slot_size, iy + item_slot_size,
            ...
            if item:
                ...
                # Small rarity dot — only indicator that something is here
                self.canvas.create_oval(
                    x + item_slot_size - 9, iy + 2,
                    x + item_slot_size - 2, iy + 9,
                    fill=item.get_color(), outline=''
                )
                cnt = getattr(item,'count',1)
                self.canvas.create_text(
                    x + item_slot_size // 2, iy + item_slot_size // 2,
                    text=emoji, font=('Arial', 14)
                )
                if cnt > 1:
                    self.canvas.create_text(
                        x + item_slot_size - 4, iy + item_slot_size - 4,
                        text=str(cnt), fill='white',
                        font=('Arial', 7, 'bold'), anchor='se'
                    )

            # Key label in corner — small and unobtrusive
            lbl_col = '#ffffff' if selected else '#8888aa'
            self.canvas.create_text(
                x + 6, iy + 7, text=labels[i],
                fill=lbl_col, font=('Arial', 7, 'bold')
            )

                # ── Tooltip for hovered item slot ─────────────────────────────────────
        mx, my = self.mouse_pos
        for i in range(3):
            x = ix0 + i * (item_slot_size + item_gap)
            if x <= mx <= x + item_slot_size and iy <= my <= iy + item_slot_size:
                item = self.hotbar_items[i]
                if item:
                    tip   = f"{item.name}\n{item.get_description()}"
                    lines = tip.split('\n')
                    line_h = 14
                    tip_h  = len(lines) * line_h + 10
                    tip_w  = max(len(l) for l in lines) * 7 + 16
                    tx = max(4, min(mx, WINDOW_W - tip_w - 4))
                    ty = iy - 6
                    # Tooltip box also semi-transparent
                    self.canvas.create_rectangle(
                        tx, ty - tip_h, tx + tip_w, ty,
                        fill='#000033', outline='#6666cc', width=1, stipple='gray75'
                    )
                    self.canvas.create_rectangle(
                        tx, ty - tip_h, tx + tip_w, ty,
                        fill='', outline='#6666cc', width=1
                    )
                    for j, line in enumerate(lines):
                        self.canvas.create_text(
                            tx + 6, ty - tip_h + 5 + j * line_h,
    ...
    def draw_minimap(self):
        ...
        pw = mc.winfo_width()
        ph = mc.winfo_height()
        if pw < 2:   # canvas not yet laid out
            return

        # Dark background (already the canvas bg, but draw a border line)
        mc.create_line(0, 0, 0, ph, fill='#333355', width=2)
        mc.create_text(pw // 2, 12, text='MAP', fill='#5555aa',
                       font=('Arial', 10, 'bold'))

        if not self._player_has_map():
            mc.create_text(pw // 2, ph // 2,
                           text='No map\nequipped',
                           fill='#444466', font=('Arial', 10), justify='center')
            return

        # Square map area — centred, with padding
        ms = min(pw - MAP_PAD * 2, ph - MAP_PAD * 4 - 24)
        if ms < 10:
            return
        mx = (pw - ms) // 2
        my = 24
        ...
        if self.dungeon_id == 0:
            self._draw_minimap_town(mx, my, ms, mc)
        else:
            self._draw_minimap_dungeon(mx, my, ms, mc)
    ...
    def update_player(self,dt):
        ...
        # Apply temporary speed/atk boosts from consumables
        if getattr(p,'_speed_boost',0) and now < getattr(p,'_speed_boost_end',0):
            p.speed = p.base_speed * (1 + p._speed_boost)
        if getattr(p,'_atk_boost_val',0) and now < getattr(p,'_atk_boost_end',0):
            p.atk = max(p.atk, p.atk + getattr(p,'_atk_boost_val',0))
                    ...
                    if door_rect:
                        ...
                        if (p.x + p.size > dx1 and p.x - p.size < dx2 and
                            ...
                            if indoor_npc_name:
                                for npc in self.room.npcs:
                                    if npc.name == indoor_npc_name:
                                        npc.indoor_x = WINDOW_W // 2
                                        npc.indoor_y = WINDOW_H // 3
                                        npc._indoor_target = (WINDOW_W // 2, WINDOW_H // 3)
                                        break
...
# === TOWN RENDERING ===
        ...
        if self.dungeon_id == 0:
            ...
            if self.current_interior:
                building  = self.current_interior
                btype     = building.get('type','')
                npc_name  = building.get('indoor_npc_name','')
                ow        = 24
                W, H      = WINDOW_W, WINDOW_H
                dg        = 90
                cv        = self.canvas
                ...
                # ── Animated helpers ─────────────────────────────────────
                def draw_fire(fcx, fcy, fw=30, fh=40, t=0):
                    fl = abs(math.sin(t*8))*5
                    cv.create_polygon(fcx-fw//2,fcy, fcx+fw//2,fcy,
                                      fcx+fw//4,fcy-fh//2, fcx,fcy-fh-int(fl),
                                      fcx-fw//4,fcy-fh//2, fill='#FF6600',outline='')
                    cv.create_polygon(fcx-fw//4,fcy, fcx+fw//4,fcy,
                                      fcx,fcy-fh//2-int(fl), fill='#FFD700',outline='')
                def draw_bubble(bcx, bcy, phase, col='#88ccff'):
                    by2=bcy-int(phase*50); br2=max(1,int(5*(1-phase*0.5)))
                    cv.create_oval(bcx-br2,by2-br2,bcx+br2,by2+br2,
                                   fill=col,outline='white',width=1)
                def draw_lantern(lcx, lcy, t=0):
                    gr=28+int(math.sin(t*3)*4)
                    cv.create_oval(lcx-gr,lcy-gr,lcx+gr,lcy+gr,
                                   fill='#ffcc44',outline='',stipple='gray25')
                    cv.create_rectangle(lcx-10,lcy-18,lcx+10,lcy+14,
                                        fill='#2a2a2a',outline='#888',width=1)
                    cv.create_oval(lcx-7,lcy-14,lcx+7,lcy+10,fill='#FFD700',outline='')
                    cv.create_line(lcx,lcy-18,lcx,lcy-30,fill='#888',width=2)

                # ── Floor regions ────────────────────────────────────────
                if btype == 'house':
                    midY, midX = H//2, W//2
                    cv.create_rectangle(ow,ow,midX,midY,     fill='#4a3a5a',outline='')
                    cv.create_rectangle(midX,ow,W-ow,midY,   fill='#3a3020',outline='')
                    cv.create_rectangle(ow,midY,midX,H-ow,   fill='#5a4030',outline='')
                    cv.create_rectangle(midX,midY,W-ow,H-ow, fill='#6b5040',outline='')
                elif btype == 'inn':
                    divY,divX = int(H*0.45),int(W*0.55)
                    cv.create_rectangle(ow,ow,W-ow,divY,      fill='#5a3020',outline='')
                    cv.create_rectangle(ow,divY,divX,H-ow,    fill='#6b4a2a',outline='')
                    cv.create_rectangle(divX,divY,W-ow,H-ow,  fill='#3a2a10',outline='')
                elif btype == 'library':
                    cv.create_rectangle(ow,ow,W-ow,H-ow,      fill='#4a3828',outline='')
                elif btype == 'blacksmith':
                    midX2 = W//2
                    cv.create_rectangle(ow,ow,midX2,H-ow,     fill='#2a2a2a',outline='')
                    cv.create_rectangle(midX2,ow,W-ow,H-ow,   fill='#333322',outline='')
                elif btype == 'tower':
                    cv.create_rectangle(ow,ow,W-ow,H-ow,      fill='#1a1030',outline='')
                else:
                    floor_c = {'shop':'#3a3020'}.get(btype,'#3a3a3a')
                    cv.create_rectangle(ow,ow,W-ow,H-ow, fill=floor_c,outline='')

                # ── Outer walls ─────────────────────────────────────────
                wc = {'house':'#4a3020','inn':'#3a1800','library':'#5c3d1e',
                      'blacksmith':'#1a1a1a','tower':'#2d1060'}.get(btype,'#555555')
                cv.create_rectangle(0,0,W,ow,   fill=wc,outline='')
                cv.create_rectangle(0,H-ow,W,H, fill=wc,outline='')
                cv.create_rectangle(0,0,ow,H,   fill=wc,outline='')
                cv.create_rectangle(W-ow,0,W,H, fill=wc,outline='')

                # ── Inner walls ─────────────────────────────────────────
                for wx1,wy1,wx2,wy2 in walls_list:
                    cv.create_rectangle(wx1,wy1,wx2,wy2,fill=wc,outline='')

                # ── Exit gap ─────────────────────────────────────────────
                ex0,ex1 = W//2-dg//2, W//2+dg//2
                fc2 = {'house':'#6b5040','inn':'#6b4a2a'}.get(btype,'#4a4a3a')
                cv.create_rectangle(ex0,H-ow,ex1,H,fill=fc2,outline='')
                cv.create_text(W//2,H-ow//2,text='EXIT',fill='#ffd700',font=('Arial',8,'bold'))

                # Building name
                cv.create_text(W//2,ow//2,text=building.get('name',''),
                ...
                for obj in objs:
                    ...
                    if ot == 'collision_only':
                        pass  # invisible collision block

                    elif ot == 'label':
                        cv.create_text(ox,oy,text=obj.get('text',''),
                                       fill=obj.get('color','#888'),font=('Arial',9,'bold'))

                    elif ot == 'book':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,
                                            fill=obj.get('color','#884444'),
                                            outline='#111',width=1)
                        if obj.get('open'):  # open book
                            cv.create_line(ox+ow2//2,oy,ox+ow2//2,oy+oh2,fill='#8B5e3c',width=2)
                            for ly in range(oy+4,oy+oh2-4,6):
                                cv.create_line(ox+4,ly,ox+ow2//2-2,ly,fill='#888',width=1)
                                cv.create_line(ox+ow2//2+2,ly,ox+ow2-4,ly,fill='#888',width=1)

                    elif ot == 'shelf':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,
                                            fill='#8B5e3c',outline='#3a2010',width=1)

                    elif ot == 'bed':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,fill='#5a3a7a',outline='#3a2050',width=2)
                        cv.create_rectangle(ox,oy,ox+ow2,oy+22,fill='#8866aa',outline='#5a3a7a',width=1)
                        for px5 in [ox+10,ox+ow2//2]:
                            cv.create_oval(px5,oy+4,px5+45,oy+18,fill='white',outline='#ddd')
                        cv.create_polygon(ox,oy+22,ox+ow2,oy+22,ox+ow2,oy+50,ox,oy+55,
                                          fill='#aa88cc',outline='#7a5a9a',width=1)

                    elif ot == 'chest':
                        chx,chy = ox,oy
                        has_items = bool(self.player.chest_items)
                        cc = '#DAA520' if has_items else '#8B6914'
                        lc2 = '#FFD700' if has_items else '#A0804A'
                        cv.create_rectangle(chx,chy+20,chx+80,chy+60,fill=cc,outline='#5a3a00',width=2)
                        cv.create_rectangle(chx,chy,chx+80,chy+22,fill=lc2,outline='#5a3a00',width=2)
                        cv.create_rectangle(chx+32,chy+18,chx+48,chy+34,fill='#888',outline='#444',width=1)
                        cv.create_oval(chx+34,chy+20,chx+46,chy+32,fill='#666',outline='#333')
                        if has_items:
                            for si4 in range(5):
                                a4=now*2+si4*1.257
                                cv.create_oval(chx+40+int(math.cos(a4)*32)-3,
                                               chy+30+int(math.sin(a4)*22)-3,
                                               chx+40+int(math.cos(a4)*32)+3,
                                               chy+30+int(math.sin(a4)*22)+3,
                                               fill='#FFD700',outline='')
                        d_ch=math.hypot(self.player.x-(chx+40),self.player.y-(chy+30))
                        if d_ch < 80:
                            cv.create_text(chx+40,chy-14,text='Press F / E to Open',
                                           fill='yellow',font=('Arial',10,'bold'))

                    elif ot == 'rect':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,
                                            fill=obj.get('color','#555'),outline='#333',width=2)
                        if obj.get('label'):
                            cv.create_text(ox+ow2//2,oy+oh2//2,text=obj['label'],
                                           fill='#aaaaaa',font=('Arial',8))

                    elif ot == 'fireplace':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,fill='#5a3a2a',outline='#3a2010',width=3)
                        for ri4 in range(3):
                            for ci4 in range(2):
                                off4 = 22 if ri4%2 else 0
                                cv.create_rectangle(ox+8+ci4*55+off4,oy+12+ri4*28,
                                                    ox+54+ci4*55+off4,oy+36+ri4*28,
                                                    fill='#7a4a30',outline='#3a2010',width=1)
                        cv.create_rectangle(ox+18,oy+oh2-55,ox+ow2-18,oy+oh2-5,
                                            fill='#1a0a00',outline='#888',width=2)
                        draw_fire(ox+ow2//2,oy+oh2-6,fw=ow2-40,fh=45,t=now)
                        cv.create_rectangle(ox-6,oy+oh2-60,ox+ow2+6,oy+oh2-52,
                                            fill='#8B5e3c',outline='#3a2010',width=1)

                    elif ot == 'couch':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,fill='#8B4513',outline='#5a2a00',width=2)
                        cv.create_rectangle(ox,oy-28,ox+ow2,oy,fill='#a0522d',outline='#5a2a00',width=2)
                        for ax4 in [ox,ox+ow2-20]:
                            cv.create_rectangle(ax4,oy-28,ax4+20,oy+oh2,fill='#7a3a10',outline='#5a2a00',width=1)

                    elif ot == 'rug':
                        cv.create_oval(ox,oy,ox+ow2,oy+oh2,
                                       fill=obj.get('color','#8B0000'),outline='#600000',width=2)
                        cv.create_oval(ox+14,oy+10,ox+ow2-14,oy+oh2-10,
                                       fill='',outline='#cc2222',width=1)

                    elif ot == 'stove':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,fill='#333',outline='#1a1a1a',width=2)
                        for ki in range(2):
                            for kj in range(2):
                                kx3=ox+14+ki*40; ky3=oy+14+kj*30
                                cv.create_oval(kx3,ky3,kx3+20,ky3+20,fill='#555',outline='#888')
                        cv.create_rectangle(ox+10,oy+oh2-50,ox+ow2-10,oy+oh2-5,
                                            fill='#1a0a00',outline='#888',width=2)
                        draw_fire(ox+ow2//2,oy+oh2-6,fw=ow2-28,fh=35,t=now+0.3)

                    elif ot == 'oven':
                        # Large bakery oven
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,fill='#4a3020',outline='#2a1800',width=3)
                        # Brickwork
                        for ri5 in range(4):
                            for ci5 in range(3):
                                off5 = 28 if ri5%2 else 0
                                bx6=ox+8+ci5*52+off5; by6=oy+10+ri5*38
                                cv.create_rectangle(bx6,by6,bx6+44,by6+30,
                                                    fill='#6a4020',outline='#2a1800',width=1)
                        # Oven door
                        door_h = max(60, oh2//2)
                        cv.create_rectangle(ox+ow2//4,oy+oh2-door_h-8,
                                            ox+ow2*3//4,oy+oh2-8,
                                            fill='#1a0a00',outline='#888',width=3)
                        draw_fire(ox+ow2//2,oy+oh2-10,fw=ow2//2,fh=door_h-10,t=now)
                        # Knobs
                        for ki2 in range(3):
                            kx4=ox+20+ki2*(ow2-40)//2
                            cv.create_oval(kx4,oy+12,kx4+18,oy+30,fill='#888',outline='#444')

                    elif ot == 'pot':
                        lc5=['#44ff44','#33dd33','#55ff55'][int(now*3)%3]
                        cv.create_oval(ox-22,oy-10,ox+22,oy+28,fill='#2a2a2a',outline='#555',width=3)
                        cv.create_oval(ox-18,oy-6,ox+18,oy+16,fill=lc5,outline='')
                        for bi8 in range(3):
                            draw_bubble(ox-8+bi8*8,oy+5,(now*0.8+bi8*0.33)%1.0,col='#88ff88')

                    elif ot == 'counter' or ot == 'display_counter':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,fill='#8B5e3c',outline='#3a2010',width=2)
                        cv.create_rectangle(ox,oy,ox+ow2,oy+12,fill='#a07050',outline='#3a2010',width=1)
                        if ot == 'display_counter':
                            pc7=['#D2691E','#FFD700','#FF8C00','#CD853F','#cc4444']
                            for di2 in range(min(5,max(1,ow2//55))):
                                dx4=ox+18+di2*(ow2-36)//max(1,ow2//55-1 if ow2//55>1 else 1)
                                cv.create_oval(dx4,oy-16,dx4+42,oy,
                                               fill=pc7[di2%5],outline='#8B4513',width=1)

                    elif ot == 'table':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+16,fill='#8B5e3c',outline='#3a2010',width=2)
                        for lx8 in [ox+12,ox+ow2-18]:
                            cv.create_line(lx8,oy+16,lx8,oy+oh2,fill='#5c3d1e',width=5)

                    elif ot == 'chair':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2//2,fill='#8B5e3c',outline='#3a2010',width=2)
                        cv.create_rectangle(ox,oy-16,ox+ow2,oy,fill='#a07050',outline='#3a2010',width=1)

                    elif ot == 'candle':
                        cv.create_rectangle(ox-4,oy,ox+4,oy+16,fill='#fffacd',outline='#daa520',width=1)
                        draw_fire(ox,oy,fw=10,fh=14,t=now+0.7)

                    elif ot == 'window':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,fill='#87CEEB',outline='#888',width=1)
                        cv.create_line(ox+ow2//2,oy,ox+ow2//2,oy+oh2,fill='#888',width=1)
                        cv.create_line(ox,oy+oh2//2,ox+ow2,oy+oh2//2,fill='#888',width=1)

                    elif ot == 'barrel':
                        cv.create_oval(ox-22,oy-30,ox+22,oy+30,fill='#8B4513',outline='#5a2a00',width=2)
                        for hr4 in [-12,0,12]:
                            cv.create_line(ox-22,oy+hr4,ox+22,oy+hr4,fill='#5a2a00',width=2)

                    elif ot == 'sack':
                        # Drawn burlap sack — no emoji
                        cv.create_oval(ox-25,oy-35,ox+25,oy+35,fill='#c8b080',outline='#8B7040',width=2)
                        cv.create_line(ox,oy-35,ox,oy-45,fill='#8B7040',width=3)
                        cv.create_oval(ox-8,oy-50,ox+8,oy-38,fill='#8B7040',outline='#5a4020',width=2)
                        # Crosshatch texture lines
                        for xi in range(-20,21,10):
                            cv.create_line(ox+xi,oy-30,ox+xi+5,oy+30,fill='#8B7040',width=1)

                    elif ot == 'lantern':
                        draw_lantern(ox,oy,t=now)

                    elif ot == 'carpet':
                        # Red carpet with golden outline
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,
                                            fill=obj.get('color','#8B0000'),outline='')
                        border = 5
                        gc_col = obj.get('outline_color','#FFD700')
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,
                                            fill='',outline=gc_col,width=border)
                        cv.create_rectangle(ox+border*2,oy+border*2,
                                            ox+ow2-border*2,oy+oh2-border*2,
                                            fill='',outline=gc_col,width=1)

                    elif ot == 'desk':
                        # Reading desk — richer than plain rect
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,
                                            fill='#6B3A1F',outline='#3a2010',width=2)
                        cv.create_rectangle(ox,oy,ox+ow2,oy+10,
                                            fill='#8B5e3c',outline='#3a2010',width=1)
                        # Open book on desk
                        bk_x,bk_y = ox+ow2//2-22,oy+10
                        cv.create_rectangle(bk_x,bk_y,bk_x+44,bk_y+oh2-14,
                                            fill='#f0e8d0',outline='#8B5e3c',width=1)
                        cv.create_line(bk_x+22,bk_y,bk_x+22,bk_y+oh2-14,
                                       fill='#8B5e3c',width=2)
                        for ly3 in range(bk_y+5,bk_y+oh2-16,6):
                            cv.create_line(bk_x+3,ly3,bk_x+20,ly3,fill='#888',width=1)
                            cv.create_line(bk_x+24,ly3,bk_x+41,ly3,fill='#888',width=1)

                    elif ot == 'wall_weapon':
                        # Draw a weapon hung on the wall
                        wx5,wy5 = ox,oy
                        ang = obj.get('angle',0)
                        wt2 = obj.get('weapon','sword')
                        ca,sa = math.cos(ang),math.sin(ang)
                        def rw(dx,dy): return wx5+dx*ca-dy*sa, wy5+dx*sa+dy*ca
                        if wt2 == 'sword':
                            cv.create_line(*rw(0,-30),*rw(0,30),fill='#aaaaaa',width=4)
                            cv.create_line(*rw(-12,-2),*rw(12,-2),fill='#888888',width=3)
                            cv.create_polygon(*rw(-2,-30),*rw(2,-30),*rw(0,-45),
                                              fill='#cccccc',outline='#888888')
                        elif wt2 == 'axe':
                            cv.create_line(*rw(0,-20),*rw(0,30),fill='#8B4513',width=5)
                            cv.create_polygon(*rw(-14,-20),*rw(6,-28),*rw(6,0),*rw(-8,0),
                                              fill='#888888',outline='#555555')
                        elif wt2 == 'spear':
                            cv.create_line(*rw(0,-40),*rw(0,40),fill='#8B4513',width=3)
                            cv.create_polygon(*rw(-4,-40),*rw(4,-40),*rw(0,-58),
                                              fill='#aaaaaa',outline='#777777')

                    elif ot == 'floor_weapon':
                        # Weapon lying on the ground at an angle
                        wx6,wy6 = ox,oy
                        ang2 = obj.get('angle',0.2)
                        wt3  = obj.get('weapon','sword')
                        ca2,sa2 = math.cos(ang2),math.sin(ang2)
                        def rfl(dx,dy): return wx6+dx*ca2-dy*sa2, wy6+dx*sa2+dy*ca2
                        if wt3 == 'sword':
                            cv.create_line(*rfl(-30,0),*rfl(30,0),fill='#aaaaaa',width=5)
                            cv.create_line(*rfl(-8,-8),*rfl(-8,8),fill='#888888',width=3)
                            cv.create_polygon(*rfl(28,-3),*rfl(28,3),*rfl(42,0),
                                              fill='#cccccc',outline='#888888')
                        elif wt3 == 'axe':
                            cv.create_line(*rfl(-25,0),*rfl(25,0),fill='#8B4513',width=5)
                            cv.create_polygon(*rfl(15,-14),*rfl(28,0),*rfl(15,6),*rfl(8,0),
                                              fill='#888888',outline='#555555')
                        elif wt3 == 'spear':
                            cv.create_line(*rfl(-40,0),*rfl(40,0),fill='#8B4513',width=3)
                            cv.create_polygon(*rfl(38,-4),*rfl(38,4),*rfl(55,0),
                                              fill='#aaaaaa',outline='#777777')

                    elif ot == 'drawn_shield':
                        # Pentagon-ish shield shape
                        pts = [(ox-20,oy),(ox+20,oy),(ox+22,oy+20),(ox,oy+35),(ox-22,oy+20)]
                        flat = []
                        for px8,py8 in pts: flat += [px8,py8]
                        cv.create_polygon(flat,fill='#4a4a8a',outline='#888888',width=2)
                        cv.create_polygon(flat[2:-2]+flat[:2],fill='',outline='#aaaacc',width=1)
                        cv.create_line(ox,oy,ox,oy+35,fill='#888888',width=1)
                        cv.create_line(ox-22,oy+20,ox+22,oy+20,fill='#888888',width=1)

                    elif ot == 'big_cauldron':
                        # Detailed large cauldron with legs, handles, fire
                        cr2 = obj.get('r',40)
                        lc7=['#44ff44','#33dd33','#55ff55'][int(now*3)%3]
                        # Legs
                        for lx10,ly10 in [(ox-cr2+8,oy+cr2*0.6),(ox+cr2-8,oy+cr2*0.6),
                                          (ox,oy+cr2*0.7)]:
                            cv.create_line(int(lx10),int(ly10),int(lx10-4),int(ly10+cr2*0.5),
                                           fill='#555',width=5)
                        # Bowl
                        cv.create_oval(ox-cr2,oy-cr2//2,ox+cr2,oy+cr2,
                                       fill='#222222',outline='#555555',width=3)
                        # Rim
                        cv.create_oval(ox-cr2,oy-cr2//2-4,ox+cr2,oy-cr2//2+8,
                                       fill='#333333',outline='#666666',width=2)
                        # Handles
                        for hx2 in [ox-cr2-8,ox+cr2+8]:
                            cv.create_oval(hx2-8,oy-cr2//4-8,hx2+8,oy-cr2//4+8,
                                           fill='#444444',outline='#666',width=2)
                        # Liquid
                        cv.create_oval(ox-cr2+6,oy-cr2//2+6,ox+cr2-6,oy+cr2//3,
                                       fill=lc7,outline='')
                        # Bubbles
                        for bi10 in range(5):
                            draw_bubble(ox-cr2//2+bi10*cr2//4, oy-cr2//4,
                                        (now*0.8+bi10*0.2)%1.0, col='#88ff88')
                        # Fire underneath
                        draw_fire(ox,oy+cr2*0.9,fw=cr2,fh=cr2//2,t=now+float(cr2)*0.01)

                    elif ot == 'bread_loaf':
                        # Simple drawn bread loaf
                        cv.create_oval(ox-25,oy-10,ox+25,oy+18,fill='#D2691E',outline='#8B4513',width=2)
                        cv.create_oval(ox-18,oy-16,ox+18,oy-2,fill='#D2691E',outline='#8B4513',width=2)
                        # Scoring lines
                        for xi2 in range(-12,13,8):
                            cv.create_line(ox+xi2,oy-14,ox+xi2+3,oy+16,
                                           fill='#A0522D',width=1)

                    elif ot == 'potion':
                        pc8 = obj.get('color','#4488ff')
                        cv.create_oval(ox-10,oy-6,ox+10,oy+14,fill=pc8,outline='#333',width=1)
                        cv.create_rectangle(ox-5,oy-14,ox+5,oy-4,fill='#aaa',outline='#555',width=1)
                        cv.create_rectangle(ox-6,oy-19,ox+6,oy-12,fill='#8B4513',outline='#5a2a00',width=1)

                    elif ot == 'cauldron':
                        lc6=['#44ff44','#33dd33','#55ff55'][int(now*3)%3]
                        cv.create_oval(ox-35,oy,ox+35,oy+60,fill='#2a2a2a',outline='#555',width=3)
                        cv.create_oval(ox-29,oy+6,ox+29,oy+38,fill=lc6,outline='')
                        for bi9 in range(5):
                            draw_bubble(ox-16+bi9*8,oy+18,(now*0.8+bi9*0.2)%1.0,col='#88ff88')
                        draw_fire(ox,oy+58,fw=50,fh=40,t=now)
                        for lx9 in [ox-22,ox+12]:
                            cv.create_line(lx9,oy+52,lx9-4,oy+74,fill='#555',width=4)

                    elif ot == 'forge':
                        cv.create_rectangle(ox,oy,ox+ow2,oy+oh2,fill='#5a3a2a',outline='#3a2010',width=3)
                        for ri6 in range(4):
                            for ci6 in range(2):
                                off6 = 26 if ri6%2 else 0
                                bx7=ox+8+ci6*70+off6; by7=oy+10+ri6*38
                                cv.create_rectangle(bx7,by7,bx7+60,by7+30,
                                                    fill='#7a4a30',outline='#3a2010',width=1)
                        cv.create_rectangle(ox+ow2//4,oy+oh2-70,ox+ow2*3//4,oy+oh2-8,
                                            fill='#1a0a00',outline='#888',width=2)
                        draw_fire(ox+ow2//2,oy+oh2-10,fw=ow2//2,fh=55,t=now)

                    elif ot == 'anvil':
                        cv.create_rectangle(ox-15,oy+30,ox+55,oy+48,fill='#333',outline='#111',width=2)
                        cv.create_polygon(ox-5,oy+30,ox+45,oy+30,ox+35,oy,ox+5,oy,
                                          fill='#444',outline='#111',width=2)
                        cv.create_polygon(ox+35,oy+14,ox+35,oy+24,ox+70,oy+21,
                                          fill='#444',outline='#111',width=2)
                        cv.create_rectangle(ox+5,oy,ox+35,oy+14,fill='#666',outline='#111',width=1)

                    elif ot == 'gem_display':
                        gc = obj.get('color','#ff4444')
                        gn = obj.get('name','')
                        cv.create_polygon(ox,oy, ox+14,oy+10, ox,oy+20, ox-14,oy+10,
                                          fill=gc,outline='white',width=1)
                        if gn:
                            cv.create_text(ox,oy+28,text=gn,fill='#aaaaaa',font=('Arial',7))
                        # Sparkle
                        for si5 in range(4):
                            a5=now*2+si5*1.571
                            cv.create_oval(ox+int(math.cos(a5)*16)-2,oy+10+int(math.sin(a5)*10)-2,
                                           ox+int(math.cos(a5)*16)+2,oy+10+int(math.sin(a5)*10)+2,
                                           fill=gc,outline='')

                    # ── Tower special decorations (animated) ──────────────
                    if btype == 'tower' and ot not in ('label',):
                        pass  # all drawn via objects above

                # ── Tower magic circle (always drawn on top) ──────────────
                if btype == 'tower':
                    mcx,mcy = W//2,H//2+20
                    for radius, col in [(110,'#220044'),(90,'#330066'),(70,'#440088')]:
                        cv.create_oval(mcx-radius,mcy-radius,mcx+radius,mcy+radius,
                                       fill='',outline=col,width=3)
                    for i2 in range(8):
                        angle2=now*1.2+i2*math.pi/4
                        runes2=['✦','★','◆','⬡','⬟','✧','⬢','◇']
                        cv.create_text(mcx+int(math.cos(angle2)*90),
                                       mcy+int(math.sin(angle2)*90),
                                       text=runes2[i2],fill='#aa44ff',font=('Arial',12))
                    pulse2=abs(math.sin(now*2))*20
                    cv.create_oval(mcx-30-int(pulse2),mcy-30-int(pulse2),
                                   mcx+30+int(pulse2),mcy+30+int(pulse2),
                                   fill='#220044',outline='#8833ff',width=2)
                    cv.create_oval(mcx-12,mcy-12,mcx+12,mcy+12,fill='#dd88ff',outline='')
                    cbx,cby = W-ow-80,ow+130
                    gp=int(abs(math.sin(now*1.5))*15)
                    cv.create_oval(cbx-38-gp,cby-38-gp,cbx+38+gp,cby+38+gp,
                                   fill='',outline='#6622aa',width=2)
                    cv.create_oval(cbx-38,cby-38,cbx+38,cby+38,
                                   fill='#220033',outline='#8833ff',width=3)
                    for i3 in range(3):
                        a3=now*2+i3*2.09
                        cv.create_oval(cbx+int(math.cos(a3)*22)-7,
                                       cby+int(math.sin(a3)*22)-7,
                                       cbx+int(math.cos(a3)*22)+7,
                                       cby+int(math.sin(a3)*22)+7,
                                       fill='#cc66ff',outline='')
                    cv.create_rectangle(cbx-12,cby+38,cbx+12,cby+60,
                                        fill='#5a3a7a',outline='#3a2050',width=2)
                    cv.create_rectangle(cbx-25,cby+55,cbx+25,cby+65,
                                        fill='#5a3a7a',outline='#3a2050',width=2)
                    for i4,oc3 in enumerate(['#ff44ff','#44ffff','#ffff44','#ff6644']):
                        ang3=now*0.8+i4*math.pi/2
                        cv.create_oval(W//2+int(math.cos(ang3)*170)-10,
                                       H//2+int(math.sin(ang3)*120)-10,
                                       W//2+int(math.cos(ang3)*170)+10,
                                       H//2+int(math.sin(ang3)*120)+10,
                                       fill=oc3,outline='white',width=1)
                ...
                if indoor_npc_name:
                    for npc in self.room.npcs:
                        if npc.name == indoor_npc_name:
                            indoor_npc = npc; break
                if indoor_npc:
                    nx,ny = int(indoor_npc.indoor_x),int(indoor_npc.indoor_y)
                    nr3=18
                    cv.create_oval(nx-nr3,ny-nr3,nx+nr3,ny+nr3,
                                   fill=indoor_npc.color,outline='white',width=2)
                    cv.create_text(nx,ny,text=indoor_npc.name[0].upper(),
                                   fill='black',font=('Arial',11,'bold'))
                    cv.create_text(nx,ny-nr3-14,text=indoor_npc.name,
                                   fill='white',font=('Arial',10,'bold'))
                    if math.hypot(self.player.x-nx,self.player.y-ny)<70:
                        cv.create_text(nx,ny+nr3+16,text='Press C to Talk',
                ...
                # ── Player ─────────────────────────────────────────────────
                plr=self.player; sz=plr.size
                pc={'Warrior':'red','Mage':'blue','Rogue':'purple','Cleric':'yellow',
                    'Druid':'green','Monk':'orange','Ranger':'brown'}.get(plr.class_name,'white')
                cv.create_oval(plr.x-sz-2,plr.y-sz-2,plr.x+sz+2,plr.y+sz+2,fill='white')
                cv.create_oval(plr.x-sz,plr.y-sz,plr.x+sz,plr.y+sz,fill=pc)
                cv.create_text(plr.x,plr.y,text=plr.name[0].upper(),
                               fill='black',font=('Arial',9,'bold'))

                cv.create_text(W//2,H-ow-16,text='Walk into the bottom gap to exit',
                               fill='#666655',font=('Arial',9))
                ...
                if pattern == 'brick' and building['type'] in ['house', 'library']:
                    ...
                    for i in range(5):
                        ...
                        self.canvas.create_line(bx, y_pos, bx + bw, y_pos, 
                                              fill=brick_color, width=1)
                    for i in range(3):
                        for j in range(4):
                            x_pos = bx + (bw // 4) * j + (bw // 8 if i % 2 else 0)
                            y_start = by + (bh // 6) * i
                            y_end = by + (bh // 6) * (i + 1)
                            if x_pos < bx + bw:
    ...
    # Continue with summons drawing...
        ...
        # --- Coin counter ---
        self.canvas.create_text(BAR_X, 75, anchor='nw',
                                text=f'💰 {self.player.coins}',
                                fill='#FFD700', font=('Arial', 9, 'bold'))
...
# Inventory button hint
    ...
    def loop(self):
        ...
        self.draw()
        if not self.current_interior:   # hide hotbar inside buildings
            self.draw_hotbar()
        if self.show_stats:
