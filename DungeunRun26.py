...
class Room:
    ...
    def create_town_layout(self):
        ...
        # Blacksmith Forge (top-right) - darker with smoke
        self.buildings.append({
            'type': 'blacksmith',
            'x': 1000, 'y': 200,
        ...
        # Enchanter Tower (center-left) - tall purple tower
        self.buildings.append({
            'type': 'tower',
            'x': 200, 'y': 500,
            'width': 80, 'height': 180,
        ...
        # Bakery/Inn (center-right) - warm orange color
        self.buildings.append({
            'type': 'inn',
            'x': 850, 'y': 480,
            'width': 140, 'height': 95,
        ...
        # Jeweler (bottom-left) - sparkly pink
        self.buildings.append({
            'type': 'shop',
            'x': 220, 'y': 780,
            'width': 100, 'height': 85,
        ...
        # === DECORATIONS ===
        # Better street lamps (with posts)
        lamp_positions = [(300, 350), (700, 350), (300, 650), (700, 650), (500, 500)]
        ...
        for i in range(25):
            ...
            if random.random() < 0.5:
                ...
                y = random.choice([random.randint(20, 80), random.randint(1020, 1080)])
            else:
                # Left/right edges
                x = random.choice([random.randint(20, 80), random.randint(1320, 1380)])
                y = random.randint(50, 1050)
        ...
        # Dungeon portals
        dungeon_entrances = [
            {'x': 100, 'y': 400, 'id': 1, 'name': '🌲 Forest Temple', 'color': '#228B22'},
            {'x': 1300, 'y': 250, 'id': 2, 'name': '🌋 Volcano', 'color': '#FF4500'},
            {'x': 1300, 'y': 650, 'id': 3, 'name': '❄️ Ice Cavern', 'color': '#00CED1'},
            {'x': 650, 'y': 1000, 'id': 4, 'name': '👻 Shadow Realm', 'color': '#8B008B'}
        ]
    ...
    def draw(self):
        ...
        if self.dungeon_id == 0:
            ...
            for road_y in [250, 550, 850]:
                ...
                self.canvas.create_rectangle(0, screen_y - road_width//2, WINDOW_W, screen_y + road_width//2, fill=road_color, outline='')
                # Road edge lines
                self.canvas.create_line(0, screen_y - road_width//2, WINDOW_W, screen_y - road_width//2, fill='#654321', width=2)
                self.canvas.create_line(0, screen_y + road_width//2, WINDOW_W, screen_y + road_width//2, fill='#654321', width=2)
            
            # Vertical roads
            for road_x in [300, 700]:
                screen_x = road_x - self.camera_x
                self.canvas.create_rectangle(screen_x - road_width//2, 0, screen_x + road_width//2, WINDOW_H, fill=road_color, outline='')
                self.canvas.create_line(screen_x - road_width//2, 0, screen_x - road_width//2, WINDOW_H, fill='#654321', width=2)
                self.canvas.create_line(screen_x + road_width//2, 0, screen_x + road_width//2, WINDOW_H, fill='#654321', width=2)
            ...
            for building in self.room.buildings:
                ...
                # Draw building body with simple brick effect (using stipple - fast!)
                self.canvas.create_rectangle(bx, by, bx + bw, by + bh, 
                                            fill=building['color'], outline='black', width=3)
                
                # Simple brick lines (much faster - only 10-15 lines per building)
                brick_color = '#654321'
                for i in range(5):  # Just 5 horizontal lines
                    y_pos = by + (bh // 6) * (i + 1)
                    self.canvas.create_line(bx, y_pos, bx + bw, y_pos, 
                                          fill=brick_color, width=1)
                for i in range(3):  # Just 3 vertical lines per row
                    for j in range(4):
                        ...
                        if x_pos < bx + bw:
                            self.canvas.create_line(x_pos, y_start, x_pos, y_end,
                                                  fill=brick_color, width=1)
                # Draw roof (triangle for houses, flat for others)
                if building['type'] in ['house', 'shop', 'inn']:
                    roof_points = [
                        bx, by,                    # left bottom
                        bx + bw//2, by - 30,       # top center
                        bx + bw, by                # right bottom
                    ]
                ...
                # Building name/sign
                self.canvas.create_text(bx + bw//2, by - 40, text=building['name'],
    ...
    # Continue with summons drawing...
            ...
            for deco in self.room.decorations:
                if deco['type'] == 'tree':
                    ...
                    y = deco['y'] - cam_y
                    size = deco['size']
                    # Better tree - brown trunk with leafy top
                    self.canvas.create_rectangle(x-4, y-size//2, x+4, y+size//2, fill='#654321', outline='')
                    self.canvas.create_oval(x-size, y-size*1.5, x+size, y-size//2, fill='#228B22', outline='#1B5E20')
                    ...
                    # Center water pillar - PERFECTLY CENTERED
                    pillar_width = 6
                    pillar_height = 35
                    self.canvas.create_rectangle(x-pillar_width, y-pillar_height, 
                                                 x+pillar_width, y, 
                    ...
                    for i in range(20):
                        ...
                        height = random.uniform(pillar_height, pillar_height + 20)
                        px = x + math.cos(angle) * dist
                        py = y - height + math.sin(time.time() * 3 + i) * 10  # Animated up/down
                        droplet_size = random.uniform(2, 3)
