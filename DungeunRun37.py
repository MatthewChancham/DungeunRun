...
class Player:
    ...
    def populate_skills(self):
        ...
        if self.class_name=='Mage':
            ...
            self.skills.append({'skill': blink,'name':'Blink','key':0,'level':1,'cooldown':1,'last_used':0,'cooldown_mod':1.0})
...
class GameFrame(tk.Frame):
    ...
    def refresh_active_skills(self):
        ...
        # Live cooldown ticker - refreshes every 50ms, 5 decimal places
        def _tick_cooldowns():
            try:
                ...
                for lbl, sk in cd_labels:
                    ...
                    eff_cd     = base_cd * mod          # actual cooldown after modifiers
                    last_used  = sk.get('last_used', 0)
                    remaining  = eff_cd - (now - last_used)
                    mod_str    = f" ×{mod:.2f}" if mod != 1.0 else ""
                    if remaining <= 0:
                        lbl.config(
                            text=f"Key: {sk['key']} | CD: {base_cd:.5f}s{mod_str} → {eff_cd:.5f}s  \u2705 READY",
                            fg="#44ff88")
                    else:
                        lbl.config(
                            text=f"Key: {sk['key']} | CD: {base_cd:.5f}s{mod_str} → {eff_cd:.5f}s  \u23f3 {remaining:.5f}s",
                            fg="#ff8844")
    ...
    def _get_interior_layout(self, building):
        ...
        # ── TRADER ─────────────────────────────────────────────────────────
            ...
            # ── Barrels bottom corners, clear of exit ─────────────────────
            for bx3,by3 in [(ow+25,H-ow-95),(ow+95,H-ow-80),
                             (W-ow-80,H-ow-95),(W-ow-150,H-ow-80)]:
                if safe(bx3-28,by3-40,56,80):
                    objs.append({'type':'barrel','x':bx3,'y':by3,
                                 'collision':col(bx3-28,by3-35,56,70)})
        ...
        # ── BAKERY ─────────────────────────────────────────────────────────
            ...
            objs+=[{'type':'kitchen_counter','x':ow+20,'y':pc_y,'w':W-ow*2-40,'h':45,'collision':col(ow+20,pc_y,W-ow*2-40,45)},
                   {'type':'pot','x':ow+80,'y':pc_y-2},
                   {'type':'pot','x':W-ow-90,'y':pc_y-2}]
    ...
    def draw_hotbar(self):
        """Skill hotbar: top-left of game canvas (original design).
        Consumable hotbar: right panel (map_canvas), below the map."""
        p   = self.player
        now = time.time()

        # ── SKILL hotbar — top-left of game canvas (original position & style) ──
        slot_size = 44
        gap       = 6
        start_x   = 10
        y_skill   = 80
        ...
        # ── CONSUMABLE hotbar — on map_canvas (right panel) ──────────────────
        mc  = self.map_canvas
    ...
    def draw_minimap(self):
        ...
        BUFF_H   = len(buffs) * 38 + (16 if buffs else 0)
        ...
        # ── Active buff display (above skill hotbar) ───────────────────────
        if buffs:
            by_start = ph - bottom_reserved + 4
            mc.create_text(pw//2, by_start, text='BUFFS', fill='#aaaaaa',
                           font=('Arial', 7, 'bold'))
            for i, buf in enumerate(buffs):
                remaining = buf['end'] - now
                by = by_start + 14 + i * 38
                # Background row
                mc.create_rectangle(4, by, pw-4, by+34,
                                    fill='#1a1a2a', outline='#333355', width=1)
                # Emoji icon
                mc.create_text(16, by+17, text=buf['emoji'], font=('Arial', 14))
                # Name + description
                mc.create_text(28, by+8, text=buf['name'][:14],
                               fill='#ffd700', font=('Arial', 7, 'bold'), anchor='w')
                mc.create_text(28, by+20, text=buf['desc'][:18],
                               fill='#aaaaaa', font=('Arial', 6), anchor='w')
                # Timer bar
                dur   = buf['end'] - (buf['end'] - buf.get('duration', buf['end']))
                frac  = min(1.0, remaining / max(1, buf.get('duration', 30)))
                bar_w = pw - 36
                mc.create_rectangle(28, by+27, 28+bar_w, by+33,
                                    fill='#333333', outline='')
                mc.create_rectangle(28, by+27, 28+int(bar_w*frac), by+33,
                                    fill='#44aa44', outline='')
                mc.create_text(pw-6, by+8, text=f"{remaining:.0f}s",
                               fill='#88ff88', font=('Arial', 6), anchor='e')
