...
SKILL_TREES = {
    ...
    'Mage': [
        ...
        {'name': 'Mage Armour',             'tier': 3, 'prereq': ['Mana Bolt'],             'cost': 4, 'type': 'passive',  'desc': 'PASSIVE: Energy shield equal to Willpower×5. Scales with mag.', 'branch': 'center', 'passive': None},
    ],
    'Rogue': [
    ...
    'Druid': [
        ...
        {'name': 'Barkskin',               'tier': 3, 'prereq': ['Thorn Whip'],             'cost': 4, 'type': 'passive',  'desc': 'PASSIVE: Natural armour shield equal to Wisdom×5. Scales with wis.', 'branch': 'center', 'passive': None},
    ],
    'Monk': [
...
class Enemy:
    ...
    def update(self, game):
        ...
        # ── Immobile totems: skip all movement, just use skills ───────────────
...
class Boss(Enemy):
    ...
    def _gs_do_swing(self, dt, game, now, player):
        ...
        if not self.gs_swing_hit and -0.3 <= self.gs_swing_angle <= 1.0:
            hit_radius = 95   # large swing hitbox
            if distance((self.x, self.y), (game.player.x, game.player.y)) < hit_radius:
                game.damage_player(self.atk * 3.0)
                self.gs_swing_hit = True
            # Delete player projectiles caught in the swing arc
            for proj in list(game.projectiles):
                if proj.owner in ('player', 'summon'):
                    if distance((self.x, self.y), (proj.x, proj.y)) < hit_radius + 20:
                        game.projectiles.remove(proj)
    ...
    def _start_charge(self, now, player):
        ...
        # Direction toward player at charge start
        ang = math.atan2(player.y - self.y, player.x - self.x)
        self.gs_charge_vx   = math.cos(ang) * 14
        self.gs_charge_vy   = math.sin(ang) * 14
        self.gs_state_end   = now + 0.55
    ...
    def _gs_do_charge(self, dt, game, now, player):
        ...
        if not self.gs_charge_hit:
            d = distance((self.x, self.y), (player.x, player.y))
            if d < self.size + player.size + 20:
                # Hit! Knockback player
                ang = math.atan2(player.y - self.y, player.x - self.x)
                player.x += math.cos(ang) * 140
                player.y += math.sin(ang) * 140
                player.x = clamp(player.x, player.size, WINDOW_W - player.size)
                player.y = clamp(player.y, player.size, WINDOW_H - player.size)
                game.damage_player(self.atk * 2.5)
    ...
    def _start_rapid_swing(self, now, player):
        ...
        self.gs_rapid_stage = 0       # 0..3 swings
        self.gs_state_end   = now + 2.2
    ...
    def _gs_do_rapid_swing(self, dt, game, now, player):
        ...
        # Each full rotation fires one large grey slash directly at the player
        if self.gs_swing_angle >= math.pi and self.gs_rapid_stage < 4:
            self.gs_rapid_stage += 1
            self.gs_swing_angle -= 2 * math.pi
            ang = math.atan2(player.y - self.y, player.x - self.x)
            game.spawn_projectile(self.x, self.y, ang, 8, 3.5, 32,
                                  '#888888', self.atk * 2.2, 'enemy', stype='slash')
    ...
    def _gs_phase3(self, dt, game, now, player):
        ...
        # Spinning
        self.gs_swing_angle += 9.0 * dt   # fast spin
        ...
        # Move toward player
        ang = math.atan2(player.y - self.y, player.x - self.x)
        self.x += math.cos(ang) * self.spd * 0.7
        self.y += math.sin(ang) * self.spd * 0.7
        self.x = clamp(self.x, self.size, WINDOW_W - self.size)
        self.y = clamp(self.y, self.size, WINDOW_H - self.size)
    ...
    if dungeon_id == 1:
        ...
        def make_healer_bolt(totem_ref, boss_ref):
            def healer_bolt(totem, game):
                # Fire a slow healing bolt toward the boss
                ...
                # Attach heal amount to projectile so update_entities can apply it
                if proj:
                    proj.ptype    = 'boss_heal'
                    proj.heal_amt = boss_ref.max_hp * 0.012   # heals 1.2% max HP per bolt
                    proj.boss_ref = boss_ref
            return healer_bolt

        for cx, cy in corners:
            totem = Enemy("Healer Totem", 9999, 0, 0, cx, cy, role="melee", skills=[])
            totem.base_spd = 0
            totem.spd      = 0
...
class GameFrame(tk.Frame):
    ...
    def update_entities(self,dt):
        ...
        # ── Boss-heal projectiles: apply healing when they reach the boss ─────
...
# === TOWN RENDERING ===
        ...
        if self.dungeon_id == 0:
            ...
            size = 12
            # Iron Guard: override player colour to steel grey
    ...
    # Continue with summons drawing...
            ...
            size = 12
            # Iron Guard: override player colour to steel grey in dungeon too
            if getattr(self.player, '_iron_guard_active', False):
                player_color = '#888888'
            else:
                player_color = CLASS_COLORS.get(self.player.class_name, "cyan")
        ...
        for proj in self.projectiles:
            ...
            if proj.stype == 'arrow':
                ...
                # Flying greatsword — mini silhouette aligned to travel direction
                _gsa = proj.angle
                _gca = math.cos(_gsa); _gsas = math.sin(_gsa)
                _gpa = math.cos(_gsa + math.pi/2); _gps = math.sin(_gsa + math.pi/2)
                _z = r * 1.1
                _b1x = x + _gpa*_z*0.5;  _b1y = y + _gps*_z*0.5
                _b2x = x - _gpa*_z*0.5;  _b2y = y - _gps*_z*0.5
                _b3x = x + _gca*_z*3.5 - _gpa*_z*0.3; _b3y = y + _gsas*_z*3.5 - _gps*_z*0.3
                _b4x = x + _gca*_z*3.5 + _gpa*_z*0.3; _b4y = y + _gsas*_z*3.5 + _gps*_z*0.3
                _tipx = x + _gca*_z*4.2; _tipy = y + _gsas*_z*4.2
                self.canvas.create_polygon([_b1x,_b1y,_b4x,_b4y,_tipx,_tipy,_b3x,_b3y,_b2x,_b2y],
                                           fill='#888888', outline='#333333', width=2)
                self.canvas.create_line(_b1x,_b1y, _tipx,_tipy, fill='white', width=1)
                _hx = x - _gca*_z*1.6; _hy = y - _gsas*_z*1.6
                self.canvas.create_line(x, y, _hx, _hy, fill='#3d1f0a', width=5)
                _cx1 = x+_gpa*_z*1.4; _cy1 = y+_gps*_z*1.4
                _cx2 = x-_gpa*_z*1.4; _cy2 = y-_gps*_z*1.4
                self.canvas.create_line(_cx1,_cy1, _cx2,_cy2, fill='#888888', width=4)
...
# Inventory button hint
    def draw_stats_panel(self):
        ...
        buff_by_stat = {'strength': 0, 'agility': 0, 'will': 0, 'constitution': 0}
        now = time.time()
        for buf in getattr(p, 'active_buffs', []):
            if buf['end'] > now:
                ...
                buff_by_stat['constitution'] += buf.get('con', 0)
        ...
        if active_buffs:
            ...
            self.canvas.create_text(130, bby, anchor='w',
                                    ...
                                    font=('Arial', 10, 'bold'))
            for j, buf in enumerate(active_buffs[:5]):
                remaining = buf['end'] - now
                time_str = 'ACTIVE' if remaining == float('inf') or remaining > 9000 else f'{remaining:.0f}s'
                self.canvas.create_text(130, bby + 16 + j*16, anchor='w',
                                        text=f"{buf['emoji']} {buf['name']}  {time_str}",
