...
class Player:
    ...
    def populate_skills(self):
        ...
        def rapid_strike(player, game):
            ...
            player.mana -= 3
            game.spawn_particle(player.x, player.y, 20, 'yellow')
        ...
        def chi_strike(player, game):
            ...
            size = arc_radius
            blade_particle = Particle(px, py, size, 'cyan', life=0.4, rtype='blade1', angle=angle_center)
            ...
            for e in list(game.room.enemies):
                ...
                if dist <= arc_radius:
                    ...
                    if diff < arc_width/2 or diff > math.pi*2 - arc_width/2:
                        game.damage_enemy(e, player.vit * 3.0)
        def strike(player, game):
            ...
            # Spawn blade particle WITH ANGLE
            size = arc_radius
            blade_particle = Particle(px, py, size, 'red', life=0.3, rtype='blade1', angle=angle_center)
            game.particles.append(blade_particle)
            ...
            for e in list(game.room.enemies):
                ...
                if dist <= arc_radius:
                    ...
                    diff = (angle_to_enemy - angle_center + math.pi*2) % (math.pi*2)
                    if diff < arc_width/2 or diff > math.pi*2 - arc_width/2:
                        game.damage_enemy(e, player.atk * 2.0)
        ...
        def mana_shield(player, game):
            mana_cost_per_second = 0.0000000000000001
            ...
            if player.mana < 0.5: return
            player.mana -= 0.5

            # Spawn outlined shield particle (white, outline only, life=1 sec)
        ...
        if self.class_name=='Mage':
            ...
            self.skills.append({'skill': mana_shield,'name':'Mana Shield','key':2,'level':1,'cooldown':0,'last_used':0,'cooldown_mod':1.0})
...
class Particle:
    def __init__(self, x, y, size, color, life=0.5, rtype='basic', angle=0.0, outline=False):
        self.x = x
        self.y = y
        self.size = size
        self.color = color
        ...
        self.angle = angle        # direction (used for blade rotation)
