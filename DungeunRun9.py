...
class Player:
    ...
    def populate_skills(self):
        def summon_wolf(player, game):
            ...
            player.mana -= 10
            wolf = Summoned("Wolf", hp=30, atk=5, spd=3, x=player.x+20, y=player.y+20, duration=15, owner=player)
        ...
        def fist_blast(player, game):
            ...
            game.spawn_projectile(player.x, player.y, ang, 6, 1.0, 8, 'brown', player.atk*5, stype='slash2')
        ...
        if self.class_name=='Mage':
            ...
            self.skills.append({'skill': mana_shield,'name':'Mana Shield','key':2,'level':5,'cooldown':2,'last_used':0,'cooldown_mod':1.0})
            self.skills.append({'skill': fireball,'name':'Fireball','key':3,'level':10,'cooldown':1.5,'last_used':0,'cooldown_mod':1.0})
            self.skills.append({'skill': chain_lightning,'name':'Chain Lightning','key':4,'level':15,'cooldown':2,'last_used':0,'cooldown_mod':1.0})
            ...
            self.skills.append({'skill': rapid_strike,'name':'Rapid Strike','key':3,'level':15,'cooldown':0,'last_used':0,'cooldown_mod':1.0})
            ...
            self.skills.append({'skill': shadow_dagger,'name':'Shadow Dagger','key':2,'level':5,'cooldown':0.2,'last_used':0,'cooldown_mod':1.0})
...
class Summoned:
    def __init__(self, name, hp, atk, spd, x, y, duration=10.0, role="melee", owner=None):
        self.name = name
        self.max_hp = hp
        self.hp = hp
        self.atk = atk
...
class Enemy:
    ...
    def update(self, game):
        ...
        if hasattr(self, "_last_dodge_time"):
            can_dodge = (now - self._last_dodge_time) > 0.5
        else:
        ...
        if self.role == "melee":
            if self.hp <= self.max_hp / 2:
                ...
                self.x += math.cos(ang) * (self.spd * 0.8)
                self.y += math.sin(ang) * (self.spd * 0.8)
            else:
                # chase until close
            ...
            desired_range = self.attack_range + 60  # preferred spacing
            if d < self.attack_range:  # too close → back away
                ang = math.atan2(self.y - player.y, self.x - player.x)
                self.x += math.cos(ang) * self.spd
                self.y += math.sin(ang) * self.spd
...
def create_enemy_types_by_dungeon():
    return {
        1: [  # Dungeon 1: Forest
            lambda x, y: Enemy(
                "Swordman", 30, 5, 3, x, y, role="melee",
                skills=[
