...
class Player:
    ...
    def populate_skills(self):
        def howl(summon, game):
            ...
            # Use summon.owner if you want the player’s stats
            ...
            ang = math.atan2(target.y - summon.y, target.x - summon.x)

            # Parameters for howl projectile
            speed = 10
            life = 4
            radius = 16
            damage = owner.wis * 2   # scale with player wisdom

            # Fire three howl projectiles forward
            for i in range(3):
        ...
        if self.class_name=='Mage':
            ...
            self.skills.append({'skill': summon_wolf,'name':'Nature Strike','key':1,'level':1,'cooldown':0.5,'last_used':0,'cooldown_mod':1.0})
            ...
            self.skills.append({'skill': multishot,'name':'Multishot','key':0,'level':3,'cooldown':1,'last_used':0,'cooldown_mod':1.0})
...
class Summoned:
    def __init__(self, name, hp, atk, spd, x, y, duration=10.0, role="loyal", owner=None):
        self.name = name
        self.max_hp = hp
        self.hp = hp
        self.atk = atk
        ...
        self.skills = []   # list of skill dicts, same format as player
...
class Enemy:
    ...
    def update(self, game):
        ...
        for part in game.particles:
            if part.rtype == "frost" and part.owner == "player":
                if distance((self.x, self.y), (part.x, part.y)) <= part.size * 1.2:
                    self.spd = self.base_spd * 0  # Apply slow
                    break
...
class Projectile:
    ...
    def update(self,dt,game):
        ...
        if time.time() - self.spawn_time > self.life:
            self.alive = False
        if self.stype == "howl":
            # Move forward like a normal projectile
            self.x += math.cos(self.angle) * self.speed
            self.y += math.sin(self.angle) * self.speed
                ...
                for e in list(game.room.enemies):
                    if isinstance(e, (Enemy, Boss)):
                        if distance((self.x, self.y), (e.x, e.y)) <= radius:
                            # Apply temporary slow - set to base speed * 0.3
                            e.spd = e.base_spd * 0.3  # 30% speed (70% slow)
                        else:
                            # Reset speed when outside frost
                            e.spd = e.base_spd
    ...
    def draw(self):
        ...
        for proj in self.projectiles:
            ...
            if proj.stype == 'arrow':
                ...
                # Convert projectile angle (radians) to Tkinter arc start (degrees)
                angle_deg = math.degrees(proj.angle)

                # Arc sweep (how wide the "wifi signal" looks)
                arc_extent = 30   # degrees
                thickness = 5

                # Draw 3 concentric arcs (like wifi signal lines)
                for i in range(3):
                    radius = proj.radius * (i + 2)   # increasing radius for each arc
