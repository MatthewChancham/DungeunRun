...
class Player:
    ...
    def populate_skills(self):
        ...
        if self.class_name=='Mage':
            ...
            self.skills.append({'skill': icicle,'name':'Icicle','key':3,'level':1,'cooldown':1.5,'last_used':0,'cooldown_mod':1.0})
...
class Particle:
    def __init__(self, x, y, size, color, life=0.5, rtype='basic', angle=0.0, outline=False, radius=0, owner=None, damage=0):
        self.x = x
        self.y = y
        self.size = size
        self.color = color
    ...
    def update(self, dt, game):
        ...
        # --- add this inside Particle.update() ---
            ...
            for e in list(game.room.enemies):
                if distance((self.x, self.y), (e.x, e.y)) <= radius:
                    # slow only while inside frost
                    e.spd *= 0.7

                    # damage over time
                    dmg = max(1, int(game.player.mag * 0.5))
...
class GameFrame(tk.Frame):
    ...
    def spawn_projectile(self, x, y, angle, speed, life, radius, color, damage, owner="player", ptype='normal', stype="basic"):
        ...
        return proj
    def spawn_particle(self, x, y, size, color, life, rtype, owner):
