...
class Room:
    ...
    def get_blacksmith_items(self):
        """Weapons and armor"""
        return [item for item in SHOP_ITEMS if item.item_type == 'weapon']
    ...
    def get_alchemist_items(self):
        """Stat-boosting items"""
        return [item for item in SHOP_ITEMS if item.item_type in ['ring', 'necklace']][:5]
    
    def get_chef_items(self):
        """Food items (create new temporary buff items)"""
        return []  # Can add consumable items later
    ...
    def _on_click(self, event):
        ...
        if tk.messagebox.askyesno("Unlock Skill", msg, parent=self.win):
            if self.player.unlock_tree_skill(node['name']):
                ...
                if hasattr(self.game_frame, 'refresh_active_skills'):
                    self.game_frame.refresh_active_skills()
            else:
                self.info_label.config(
...
class GameFrame(tk.Frame):
    ...
    def open_combined_skill_page(self):
        ...
        # ── Tab 1: Inventory ──────────────────────────────────────────────────
        inv_tab = tk.Frame(notebook, bg="#1a1a1a")
        notebook.add(inv_tab, text="  🎒  Inventory  ")

        # Persistent header widgets (updated in-place every tick)
        inv_header = tk.Frame(inv_tab, bg="#2a2a2a")
        inv_header.pack(fill='x', pady=8, padx=10)
        coins_label = tk.Label(inv_header, text=f"💰  Coins: {self.player.coins}",
                               font=("Arial", 16, "bold"), bg="#2a2a2a", fg="gold")
        coins_label.pack(side='left', pady=4, padx=10)
        level_label = tk.Label(inv_header,
                               text=f"⚔  Level {self.player.level}  |  "
                                    f"XP {self.player.xp}/{self.player.xp_to_next}",
                               font=("Arial", 12), bg="#2a2a2a", fg="#aaaaff")
        level_label.pack(side='left', pady=4, padx=20)

        # HP / Mana mini-bars
        bar_frame = tk.Frame(inv_tab, bg="#1a1a1a")
        bar_frame.pack(fill='x', padx=14, pady=(0, 6))
        hp_bar_bg   = tk.Frame(bar_frame, bg="#444", width=200, height=14)
        hp_bar_bg.pack(side='left', padx=(0, 4))
        hp_bar_bg.pack_propagate(False)
        hp_bar_fill = tk.Frame(hp_bar_bg, bg="#cc2222", height=14)
        hp_bar_fill.place(x=0, y=0, relheight=1.0, width=200)
        hp_bar_text = tk.Label(hp_bar_bg, text="", bg="#cc2222", fg="white",
                               font=("Arial", 8, "bold"))
        hp_bar_text.place(relx=0.5, rely=0.5, anchor='center')

        mana_bar_bg   = tk.Frame(bar_frame, bg="#444", width=200, height=14)
        mana_bar_bg.pack(side='left', padx=4)
        mana_bar_bg.pack_propagate(False)
        mana_bar_fill = tk.Frame(mana_bar_bg, bg="#2255cc", height=14)
        mana_bar_fill.place(x=0, y=0, relheight=1.0, width=200)
        mana_bar_text = tk.Label(mana_bar_bg, text="", bg="#2255cc", fg="white",
                                 font=("Arial", 8, "bold"))
        mana_bar_text.place(relx=0.5, rely=0.5, anchor='center')

        # Scrollable item list container
        inv_list_outer = tk.Frame(inv_tab, bg="#1a1a1a")
        inv_list_outer.pack(fill='both', expand=True, padx=10, pady=4)

        # Track last-known state so we only rebuild the list when something actually changed
        _inv_cache = {'coins': None, 'level': None, 'inv_sig': None,
                      'hp': None, 'mana': None}

        def rebuild_inv_list():
            """Tear down and rebuild the scrollable item rows."""
            for w in inv_list_outer.winfo_children():
                w.destroy()
            sc = tk.Canvas(inv_list_outer, bg="#1a1a1a", highlightthickness=0)
            sb = ttk.Scrollbar(inv_list_outer, orient="vertical", command=sc.yview)
            sf = tk.Frame(sc, bg="#1a1a1a")
            ...
            if not self.player.inventory:
                tk.Label(sf, text="Inventory is empty",
                         font=("Arial", 14), bg="#1a1a1a", fg="gray").pack(pady=20)
            else:
                for item in self.player.inventory:
                    item_frame = tk.Frame(sf, bg="#2a2a2a", bd=2, relief="groove")
                    item_frame.pack(fill='x', pady=5, padx=5)
                    name_text = item.name + (" ⭐" if item.soulbound else "")
                    tk.Label(item_frame, text=name_text,
                             font=("Arial", 14, "bold"), bg="#2a2a2a",
                             fg=item.get_color()).pack(anchor='w', padx=10, pady=5)
                    desc_text = item.get_description()
                    if item.soulbound:
                        desc_text += "\n[Soulbound: Stats apply even when unequipped]"
                    tk.Label(item_frame, text=desc_text, font=("Arial", 10),
                             bg="#2a2a2a", fg="white", justify='left').pack(anchor='w', padx=10, pady=2)
                    ...
                    def make_equip_cb(itm):
                        def cb():
                            if itm in self.player.equipped_items:
                                self.player.unequip_item(itm)
                            else:
                                self.player.equip_item(itm)
                            _inv_cache['inv_sig'] = None  # force rebuild
                        return cb

                    tk.Button(btn_frame, text=btn_text, bg=btn_color, fg="white",
                              font=("Arial", 10, "bold"),
                              command=make_equip_cb(item)).pack(side='left', padx=5)
                    if not item.soulbound:
                        sell_price = max(1, item.price // 2)

                        def make_sell_cb(itm, price):
                            def cb():
                                self.player.coins += price
                                self.player.remove_item_from_inventory(itm)
                                _inv_cache['inv_sig'] = None  # force rebuild
                        ...
                        tk.Button(btn_frame, text=f"Sell ({sell_price}💰)",
                                  bg="#f0ad4e", fg="white", font=("Arial", 10, "bold"),
                                  command=make_sell_cb(item, sell_price)).pack(side='left', padx=5)
        ...
        # ── Live-refresh loop (runs every 250 ms while window is open) ────────
        def auto_refresh():
            if not win.winfo_exists():
                return
            p = self.player

            # Always update coins + level header
            new_coins = p.coins
            new_level = p.level
            new_hp    = int(p.hp)
            new_mana  = int(p.mana)

            if new_coins != _inv_cache['coins']:
                _inv_cache['coins'] = new_coins
                coins_label.config(text=f"💰  Coins: {new_coins}")

            if new_level != _inv_cache['level']:
                _inv_cache['level'] = new_level
                level_label.config(text=f"⚔  Level {new_level}  |  "
                                        f"XP {p.xp}/{p.xp_to_next}")

            # HP bar
            if new_hp != _inv_cache['hp']:
                _inv_cache['hp'] = new_hp
                frac = max(0.0, min(1.0, p.hp / p.max_hp)) if p.max_hp else 0
                hp_bar_fill.place(width=int(200 * frac))
                hp_bar_text.config(text=f"HP  {new_hp}/{int(p.max_hp)}")

            # Mana bar
            if new_mana != _inv_cache['mana']:
                _inv_cache['mana'] = new_mana
                frac = max(0.0, min(1.0, p.mana / p.max_mana)) if p.max_mana else 0
                mana_bar_fill.place(width=int(200 * frac))
                mana_bar_text.config(text=f"MP  {new_mana}/{int(p.max_mana)}")

            # Rebuild inventory list only when contents actually changed
            inv_sig = (len(p.inventory),
                       tuple(id(i) for i in p.inventory),
                       tuple(id(i) for i in p.equipped_items))
            if inv_sig != _inv_cache['inv_sig']:
                _inv_cache['inv_sig'] = inv_sig
                rebuild_inv_list()

            win.after(250, auto_refresh)
    ...
    def on_key_down(self, e):
        ...
        # 1-5 selects hotbar slot (like Minecraft)
    ...
    def draw_hotbar(self):
        """Draw a Minecraft-style hotbar at the top where the old skill bar was."""
        p = self.player
        num_slots = 5
        slot_size = 50
        gap = 10   # 10+i*60 spacing matches the old bar (60 = 50 size + 10 gap)
        start_x = 10
        y = 80
        ...
        for i in range(1, num_slots + 1):
            ...
            if sk:
                ...
                if cd > 0 and elapsed < cd:
                    ...
                    self.canvas.create_rectangle(x, y, x + slot_size, y + overlay_h,
            ...
            self.canvas.create_text(x + 7, y + 7, text=str(i),
                                    fill=num_color, font=('Arial', 8, 'bold'))
...
class MainApp(tk.Tk):
    ...
    def start_game(self):
        try:
            ...
            self.geometry(f"{int(WINDOW_W + 200)}x{int(WINDOW_H + 200)}")
