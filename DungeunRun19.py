...
class Player:
    ...
    def assign_weapon(self):
        ...
        if self.class_name == "Warrior":
            ...
            self.item = Item(self.x, self.y, 'staff', 'gold', 22, owner=self)
            self.item.gem_color = 'yellow'
        elif self.class_name == "Druid":
            self.item = Item(self.x, self.y, 'staff', 'green', 22, owner=self)
            ...
            "active_skills": [
                {
                    ...
                    "cooldown_mod": sk.get('cooldown_mod', 1.0)   # <-- added
                }
...
class GameFrame(tk.Frame):
    def __init__(self,parent,player,on_quit_to_menu,dungeon_id=1):
        ...
        self.bind("t", lambda e: game.rotate_beam(2))   # rotate right
    ...
    def draw(self):
        ...
        # Player body + weapon layering
        if self.player.item:
            # Update weapon position
            if self.room.enemies:
                target = min(self.room.enemies, key=lambda e: distance((px, py), (e.x, e.y)))
                self.player.item.update(px, py, target.x, target.y)
            else:
                self.player.item.update(px, py, px + 50, py)

            # If spear or staff → draw first (beneath body)
            if self.player.item.item_type in ("spear", "staff", "sword", "dagger"):
                self.player.item.draw(self.canvas)
        CLASS_COLORS = {
            "Warrior": "red",
            "Mage": "blue",
            "Rogue": "purple",
        ...
        # If not spear/staff → draw after body (on top)
        if self.player.item and self.player.item.item_type not in ("spear", "staff"):
            self.player.item.draw(self.canvas)
        ...
        # Define a colour map for classes
        CLASS_COLORS = {
            "Warrior": "red",
            "Mage": "blue",
            "Rogue": "purple",
    ...
    def draw_stats_panel(self):
        ...
        stats = ['strength','vitality','agility','constitution','intelligence','wisdom','will']
        y_start = 120
        stat_height = 40  # taller to fit inside boxes
