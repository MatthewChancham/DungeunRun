...
class Player:
    ...
    def populate_skills(self):
        ...
        if self.class_name=='Mage':
            ...
            self.skills.append({'skill': rapid_strike,'name':'Charge','key':3,'level':15,'cooldown':2,'last_used':0,'cooldown_mod':1.0})
...
def dark_bolt(enemy, game):
    ...
    ang = math.atan2(game.player.y - enemy.y, game.player.x - enemy.x)
    game.spawn_projectile(enemy.x, enemy.y, ang, 20, 2.5, 10, 'purple', enemy.atk * 2, 'enemy')
        ...
        # --- add this inside Particle.update() ---
            ...
            for e in list(game.room.enemies):
                if distance((self.x, self.y), (e.x, e.y)) <= radius:
                    ...
                    # damage over time
                    game.damage_enemy(e, self.damage or game.player.mag * 0)
